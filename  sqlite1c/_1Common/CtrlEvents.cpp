                                  ...
                                  CBLContext* pDerivedObject)
{
	...
	if (!bSuccess)
		CBLModule::RaiseExtRuntimeError("ќшибка подключени€ событий", mmRedErr);
}
...
BOOL CCtrlEventManager::Invoke6(int nEventIndex, CValue& vP1, CValue& vP2, CValue& vP3, CValue& vP4, CValue& vP5, CValue& vP6)
{
    CValue vCtrl;
    vCtrl.AssignContext(m_pCtrlObject);
    CValue* ppValues[] = {&vCtrl, &vP1, &vP2, &vP3, &vP4, &vP5, &vP6};
    return InvokeEvent(nEventIndex, ppValues);
}
