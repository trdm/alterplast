        ...
        if (pParamName)
        {
            if (!IsWordChar(ch))
            {
                ...
                if (ch == '(')
                {
                    ...
                    {
                        m_strLastError.Format("Ошибка определения параметра \"%s\"", strParamName);
                        return FALSE;
                    }
                    pQT += nProcessed - 1;
                }
    ...
    for (int nLen = str.GetLength(); nLen > 0;)
    {
        ...
        for (int n = 0; szCurWord; szCurWord = strtok(NULL, szTrimmedSimbols), n++)
        {
            ...
            if (n == 0)
            {
                ...
                if (!pSelectListItem)
                {
                    m_strLastError.Format("Неизвестное поле ""%s"".", str);
                    return FALSE;
                }
                else
                {
                    ...
                    {
                        m_strLastError.Format("Выражения полей ключа порядка не могут иметь SQL параметров (%s).", str);
                        return FALSE;
                    }
