...
int CDelegate::CheckFindMethod(CBLContext *pCont, CString& sMethName)
{
	...
	if (nMeth == -1)
		RuntimeError("В классе '%s' не найден метод '%s'", pCont->GetTypeString(), sMethName);
...
long CDelegate::Add(CValue **params, LPCSTR key)
{
	...
	if (pCont->HasRetVal(nMeth) != m_nIsFunc)
		...
		pCont->GetTypeString(), sMethName, m_nIsFunc == 1 ? "процедура, необходима функция" : "функция, необходима процедура");
	...
	if (pCont->GetNParams(nMeth) != m_nParamCount)
	{
		...
		RuntimeError("В классе '%s' метод '%s' должен принимать %d %s!", pCont->GetTypeString(), sMethName, m_nParamCount, sParam);
	}
