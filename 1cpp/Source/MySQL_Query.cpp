...
int  CMySQL_Query::CallAsFunc(int iMethNum,class CValue & rValue,class CValue * *ppValue)
{
	return 1;
}

int  CMySQL_Query::CallAsProc(int iMethNum,class CValue * * ppValue)
{
  switch(iMethNum)
  {
    case procTest7:
      ...
      {       
        ...
        {          
          for( ; ! m_rs.IsEof(); m_rs.MoveNext() )
          {
            ...
            pBkEndUI->DoMessageLine((CString("ROW_ID = "))+m_rs["ROW_ID"], mmInformation);
            // покажем значение колонки ID
            pBkEndUI->DoMessageLine((CString("ID = "))+m_rs["ID"], mmInformation);
            // покажем значение колонки DESCR
            pBkEndUI->DoMessageLine((CString("DESCR = "))+m_rs["DESCR"], mmInformation);
...
int  CMySQL_Query::GetParamDefValue(int iMethodNum,int iParamNum,class CValue * pDefValue)const
{	
	return 0;
}
...
void  CMySQL_Query::GetExactValue(class CValue & vParam)
{
...
class CType   CMySQL_Query::GetValueType(void)const
{
	CType tType(100);
...
void  CMySQL_Query::InitObject(class CType const & tType)
{
...
int  CMySQL_Query::SaveToString(class CString & csStr)
{
	csStr = "";
	return 1;
}
...
int  CMySQL_Query::GetPropVal(int iPropNum,class CValue & rValue)const
{
	return -1;
}

int  CMySQL_Query::SetPropVal(int iPropNum,class CValue const & vValue)
{
	return -1;
}
