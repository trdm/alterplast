...
BOOL VerifyUserIB(LPCSTR szPath, LPCSTR szUserName, LPCSTR szPassword, DWORD &UserdefChecksum)
{
	...
	strFileName.Format("%s%s", ((CApp7*)AfxGetApp())->GetProps()->GetStringProp(1), "usrdef\\users.usr");
    ...
    case procAttachIB:
        {
            ...
            else //sql
            {
				if (strConnString.IsEmpty())
				{
					...
					strFileName.Format("%s%s", strPath, "1cv7.dba");
