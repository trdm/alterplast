...
CVTExtGroupings::CVTExtGroupings(CString& Groupings, CVTExtended* pVT)
{
	...
	for( int i = 0; i < sz; i++ )
	{
		...
		if( IndexParts.GetSize() != 2 )
			RuntimeError("Неверно задана группировка %i: '%s'", i+1, arrGroupings[i]);
...
{
	CNumeric** pSums = new CNumeric*[nSumCols];
	int i;

	for( i = 0; i < nSumCols; i++ )
	{
		CValue& val = pSumRow->GetValue(arrSumCols[i]);
		if( val.GetTypeCode() != NUMBER_TYPE_1C )
			val = 0L;
		pSums[i] = &(val.m_Number);
	}

	for( i = node->ArrayEqualIDs->size() - 1; i >= 0; i-- )
	{
		int nRow = node->ArrayEqualIDs->operator[](i);
		CVTExtRow* pRow = GetRow(nRow);
		for( int nSumCol = 0; nSumCol < nSumCols; nSumCol++ )
		{
			CNumeric& val = *(pSums[nSumCol]);
...
void CVTExtended::SumsBySubcontoFolders(CVTExtGroupings & Groupings, int* arrSumCols, int nSumCols, int nGrouping)
{
	...
	//переносим
	for( nRow = 0; nRow < nRowsRest; nRow++ )
	{
