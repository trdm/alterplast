...
char * m_Present;
char * m_Descr;
	char buf[0x400];	// Точно размер не знаю.
...
public:
	CFormCtrl* m_Ctrl; //указатель на поименованный атрибут формы
	CGetDoc77* m_GetDoc;
	int m_Flag;
};
...
{
public:
	 CBLExportContext(CBLExportContext const &);	//26
	 CBLExportContext(CBLContext *);	//27
	 CBLExportContext(void);	//28
	CBLExportContext &  operator=(CBLExportContext const &);	//504
...
protected:
	int  ReplaceDefValue(CValue * * *,int,CBLContext *,long);	//3227
...
class IMPORT_1C CMetaDataObjArray
{
public:
	char buf[0x20];
...
class IMPORT_1C CObjectSkipper:public CObject
{
