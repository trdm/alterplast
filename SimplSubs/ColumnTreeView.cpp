...
CColumnTreeView* CColumnTreeView::m_pView=NULL;
	...
	case CDDS_ITEMPOSTPAINT:
		{
			...
			CString sItemText = m_Tree.GetItemText(hItem);
			...
			COLORREF crTextBl = GetSysColor(COLOR_BTNTEXT);

			COLORREF crTextRed = RGB(255,0,0);
			...
			if (pNMCustomDraw->uItemState & CDIS_FOCUS) {
				...
				// Установим белый шрифт текста
				// Но только если итем у нас селектнутая(синяя)
				if ( pNMCustomDraw->uItemState & CDIS_SELECTED)
					crText = dc.SetTextColor(crText_foc_sel);
					//crText = dc.SetTextColor(crText);
				else
					crText = dc.SetTextColor(crTextRed); //Если просто в фокусе, красненький...
			} else { 				
				if (pNMCustomDraw->uItemState & CDIS_SELECTED)
					crText = dc.SetTextColor(crText);
			}
			...
			dc.SetTextColor(crTextBl); // Восстановим....
