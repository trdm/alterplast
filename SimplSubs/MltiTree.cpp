...
void CMultiTree::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	//if (CheckHit(point))		CTreeCtrl::OnLButtonDown(nFlags, point);
	OnButtonDown(TRUE, nFlags, point);
}
void CMultiTree::OnLButtonDblClk(UINT nFlags, CPoint point)
{
	//if (CheckHit(point))		CTreeCtrl::OnLButtonDblClk(nFlags, point);
	OnButtonDown(TRUE, nFlags, point);
}
...
void CMultiTree::OnButtonDown(BOOL bLeft, UINT nFlags, CPoint point)
{
	...
	if (!hItem || (nHF & (TVHT_ONITEMRIGHT|TVHT_NOWHERE|TVHT_ONITEMINDENT))
		|| (!( (nHF & TVHT_ONITEMBUTTON) || (nHF & TVHT_ONITEM) ) )
		) {
...
// required then need to implement - I'm not bothered.
		default:
			{
				if (!bCtrl && !bShift /*&& (! ((nChar == VK_LEFT) || (nChar == VK_RIGHT)) ) */) {
...
BOOL CMultiTree::OnEraseBkgnd(CDC* pDC)
{
	return TRUE;	// do nothing
}


BOOL CMultiTree::CheckHit(CPoint point)
{
	UINT fFlags;
...
void CMultiTree::OnBegindrag(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	HTREEITEM nItem = GetSelectedItem();
	if ((pNMTreeView) != NULL){ 
		CString nStrTemp[3], nStrAllDD = "";
		nStrTemp[0] = ""; // текст текущей итемы
		nStrTemp[1] = ""; // собирает все
		
		if (GetItem(&(pNMTreeView->itemNew))){ 
			if (!pAdo->m_Settings.EnableDDGroup){ 
				if (pAdo->m_Settings.EnableFullPathDD){ 
					HTREEITEM tCur, tRootItem = GetRootItem();
					tCur = pNMTreeView->itemNew.hItem;
					nStrTemp[0] = "";
					while (tCur !=tRootItem) {
						nStrTemp[0] = GetItemText(tCur);
						if (nStrTemp[0].Find("\t") != -1){ 
							nStrTemp[0] = nStrTemp[0].Left(nStrTemp[0].Find("\t"));
						}
						if (nStrTemp[1].GetLength()>0){ 
							nStrAllDD.Format("%s.%s",nStrTemp[0], nStrTemp[1]);
						} else {
							nStrAllDD.Format("%s",nStrTemp[0]);
						}
						nStrTemp[1] = nStrAllDD;
						tCur = GetParentItem(tCur);
					}
					DragImpl(nStrAllDD);

				} else {
					DragImpl(GetItemText(pNMTreeView->itemNew.hItem));
				}
				...
				if (GetSelectedCount()>1){ 
					...
					while(nItem != NULL){
						nStrTemp[0] = GetItemText(nItem);
						if (nStrTemp[0].Find("\t") != -1){ 
							nStrTemp[0] = nStrTemp[0].Left(nStrTemp[0].Find("\t"));
						}
						//nStrTemp[0] = nStrAllDD; 						if (nStrAllDD.GetLength()>0){ 					}
						if (nStrTemp[1].GetLength()>0){ 
							nStrAllDD.Format("%s%s%s",nStrTemp[1], pAdo->m_Settings.strCharsTerminator, nStrTemp[0]);
						} else {
							nStrAllDD.Format("%s",nStrTemp[0]);
...
void CMultiTree::DragImpl(CString nDrStr)
{
	CString strItem = nDrStr;
	if (strItem.Find("\t") != -1){ 
