...
---------------------------------------------------------------------
        **Синтаксис:** ReconnectNative([СтрокаПодключения])

        **Параметры:**

                - СтрокаПодключения - тип: Строка. Строка подключениея ODBC драйвера к источнику данных.
                
                В строке подключения допустимы следующие подстановки параметров подключения:
					- %server% - имя сервера;
					- %uid% - пользователя;
					- %pwd% - пароль;
					- %db% - имя БД;
					
				Строка подключения по умолчанию: "Driver={SQL Server};Server=%server%;App=1CV7;UID=%uid%;PWD=%pwd%;"	
...
---------------------------------------------------------------------
        ...
        **Параметры:**
                ...
                - Список - тип: СписокЗначений, Справочник, Счет. Объект, содержание которого будет отправляться на сервер.
                  ...
                  или план счетов), то будет попытка включить в таблицу все подчиненные элементы.
...
+---------------------------------------------------+---------------------------------------------------------------+
|{ВидДокумента|KindDocument}                        |- int - вид документа SQL формат                               |
|                                                   |- char(4) - вид документа SQL формат (база 36)                 |
+---------------------------------------------------+---------------------------------------------------------------+
|{ВидДокументаПредставление|KindDocumentPresent}    |- int - вид документа SQL формат                               |
|                                                   |- char(4) - вид документа SQL формат (база 36)                 |
+---------------------------------------------------+---------------------------------------------------------------+
|{ВидСправочника|KindReference}                     |- int - вид справочника SQL формат                             |
|                                                   |- char(4) - вид справочника SQL формат (база 36)               |
+---------------------------------------------------+---------------------------------------------------------------+
|{ВидСправочникаПредставление|KindReferencePresent} |- int - вид справочника SQL формат                             |
|                                                   |- char(4) - вид справочника SQL формат (база 36)               |
+---------------------------------------------------+---------------------------------------------------------------+
...
+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        ...
        **Поля:**
                ...
                - **Период** - тип datetime; дата начала периода, по которому происходит разворот оборотов; существует только
                  в том случае, если указана периодичность День, Неделя, Декада, Месяц, Квартал, Год;
