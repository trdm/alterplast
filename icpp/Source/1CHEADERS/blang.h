...
public:
	...
	virtual void			OnSetSpeller(CValue const &);	//125
...
//	void  `default constructor closure'(void);	//18
	...
	int  CallAsFunc(int iFuncNum,CValue & retValue,int iParamCount,CValue * *params);	//29
	...
	// Возврат: 0 - если вызов не удачный
	int  CallAsProc(int iProcNum,int iParamCount,CValue * *params);	//31
	int  Compile(void);	//34
	int  EvalExpr(char const *,CValue &,CValue * *);	//37
	int  Execute(void);	//38
	int  ExecuteBatch(char const *,CValue * *);	//39
	...
	CString   FormatValue(CValue const &,char const *);	//45
	...
	int  GetStaticVarValue(int,CValue &,int)const;	//91
	...
	int  SetStaticVarValue(int,CValue const &,int);	//138
