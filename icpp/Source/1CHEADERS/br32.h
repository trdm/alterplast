...
public:
	...
	void  GetBufferText(CString &,int,int);	//123
...
protected:
	virtual int  DrawItem(class CDC &,class CRect &,CString &,int);	//101
public:
	virtual void  GetData(CString &);	//141
	virtual int  SetData(CString &);	//385
protected:
	virtual class CWnd *  Edit(class CRect &,class CBrowse *);	//103
//public:
//	int PublicDrawItem(class CDC & DC,class CRect &Rect,CString &st,int flag)
...
//<-CGetCtrl(34h/14h)<-CCmdTarget(20h/1Ch)<-CObject(4h/4h)
...
public:
	...
	virtual void  GetCtrlText(CString &);	//139
	virtual void  GetStringData(char *,int);	//190
	virtual void  GetStringData(CString &);	//189
...
public:
	...
	virtual void  GetCtrlText(CString &);	//135
...
//<-CEditGet(40h/Ch)<-CGetCtrl(34h/14h)<-CCmdTarget(20h/1Ch)<-CObject(4h/4h)
...
public:
	...
	virtual void  GetCtrlText(CString &);	//137
...
public:
	...
	DWORD m_dwFColumn7;
	 ...
	 CFColumn(unsigned int,CString *,char const *,int,int,int);	//19
	 //(DlgCtrlID, текущее значение, ???,,,длина);
	virtual  ~CFColumn(void);	//38
	virtual void  GetData(CString &);	//142
	virtual int  SetData(CString &);	//386
...
protected:
	virtual void  GetData(CString &);	//143
	virtual int  SetData(CString &);	//387
	virtual class CWnd *  Edit(class CRect &,class CBrowse *);	//105
	virtual int  PreValid(void);	//355
};


//<-CGetDoc(A0h/4Ch)<-CDocument(54h/34h)<-CCmdTarget(20h/1Ch)<-CObject(4h/4h))
...
public:
	BYTE buf[0x20];
	CGetExchange(class CGetDoc *,int,unsigned int,CString *);	//24
	void  Fail(void);	//111
...
IMPORT_1C void __stdcall DDX_MaskText(class CDataExchange *,int,CString &,int);
...
IMPORT_1C void __cdecl DX_Text(class CGetExchange *,unsigned int,CString &);
