...
public:
	...
	static int  GetSubString(CString &,char const *,int);	//150
...
public:
	...
	void  SetCodeName(CString const &);	//243
	...
	void  SetFormul(CString const &);	//258
	...
	void  SetTitle(CString const &);	//278
	void  SetToolTip(char const *);	//282
	void  SetValType(CType &);	//286
};
...
protected:
	...
	void  AdjustTypedObj(class CCtrlHolder *,CType *);	//36
public:
	int  CalcButtonWidth(int);	//37
	int  CalcColumnWidth(int);	//38
	int  CalcEditWidth(CType const &);	//39
...
public:
	int  CreateIndirect(CString &);	//51
private:
	class CCtrlHolder *  CreatePermanentControl(class CRect const &,CType const &,char const *,char const *,long,int,int,int);	//53
protected:
	class CCtrlHolder *  CreateStaticTitle(char const *,class CRect const &);	//54
private:
	void  DefineFixedControl(CType const &,char const *,char const *,long,int);	//55
	void  DefineNewControl(CType const &,char const *,char const *,long,int,int,int);	//56
public:
...
public:
	...
	CTypedPtrArray<class CPtrArray,class CCtrlType *> *  GetCtrlArray(void);	//90
protected:
	virtual HMENU  GetDefaultMenu(void);	//99
public:
	long  GetDependParam(void);	//101
	CTypedPtrArray<class CPtrArray,class CBrowserSupport *> *  GetDocCtrlArray(void);	//102
	class CDrawMaster *  GetDrawMaster(void);	//103
	class CEditMaster *  GetEditMaster(void);	//104
	CTypedPtrArray<class CPtrArray,class CCtrlHolder *> *  GetExtCtrlArray(void);	//106
...
public:
	...
	int  LayoutControls(CTypedPtrList<class CPtrList,class CCtrlHolder *> const &,int,int);	//173
	int  LoadFromStream(struct IStream *);	//174
	int  Merge2Way(class CEditDoc *,int);	//176
	int  ModifyContol(int,CString,CString);	//177
...
public:
	void *  PlaceControl(int,void *,CString &,CType const &,char const *,char const *,int,int,int);	//213
...
protected:
	...
	int  SaveStringToArchive(class CArchive &,CString const &);	//229
...
public:
	...
	int  SetType(void *,CType const &);	//284
...
public:
	...
	int  GetItemText(class CWnd *,CString &,unsigned int);	//125
