...
public:
	...
	virtual int  GetPropVal(int,CValue &)const;	//135
	virtual int  IsPropReadable(int)const;	//183
	virtual int  SetPropVal(int,CValue const &);	//232
};
...
public:
	...
	virtual int  CallAsFunc(int,CValue &,CValue * *);	//62
	virtual int  CallAsProc(int,CValue * *);	//63
	...
	virtual int  GetParamDefValue(int,int,CValue *)const;	//126
	virtual char const *  GetPropName(int,int)const;	//134
	virtual int  GetPropVal(int,CValue &)const;	//136
	...
	virtual int  SetPropVal(int,CValue const &);	//233
...
public:
	 CRGdata(void);	//4
	 ~CRGdata(void);	//17
	void  AddCondition(CString &);	//41
	void  AddConditionName(CString &);	//42
...
public:
	...
	CType   GetVarType(char const *);	//156
...
//	void  `default constructor closure'(void);	//37
	...
	CString const &  GetName(void);	//123
	int  GetPrecision(class CRGdata *)const;	//128
	CString &  GetSentence(CString &,class CRGdata *)const;	//142
	...
	void  SetCondition(CString const &);	//201
	void  SetConditionName(CString &);	//202
	void  SetConditionNameNum(int);	//203
	void  SetHead(int);	//217
	void  SetName(CString const &);	//221
...
public:
	...
	CString &  GetSentence(CString &,class CRGdata *)const;	//143
...
public:
	...
	long  DownloadToValuesTable(CValue &,CString const &,int);	//78
	long  DownloadToValuesTable(CValue &,int,int);	//79
	int  Execute(void);	//81
	int  ExecuteRequest(void);	//82
	int  FunctionValue(CValue &,CValue * *);	//84
protected:
	int  GenerateDDName(CString &);	//85
public:
	int  Get(CValue * *);	//86
	class CRGdata *  GetData(void);	//98
	CValue const &  GetFunctionValue(int,CValue &)const;	//106
	int  GetGroupLevel(int,int);	//108
	class CRGrecord *  GetRecord(void);	//137
	class CRGset *  GetSet(void);	//145
	int  GotoPos(enum NavCtrl);	//160
	int  GroupValue(CValue &,CValue * *);	//161
	void  Initialize(char const *);	//165
	int  OrderValue(CValue &,CValue * *);	//192
...
//	void  `default constructor closure'(void);	//40
	...
	CString const &  GetName(void);	//125
	int  GetRepresentationLength(int &);	//139
	CString &  GetSentence(CString &)const;	//144
	CType &  GetType(void);	//151
...
protected:
	int  GetReferenceData(class CMetaDataObj *,CType &,char const *,int &);	//138
public:
	CString   GetString(int);	//146
