...
//size of CAdminService is 80F8h
...
public:
	...
	CFile m_file1;            //  24h + 10h
	CFile m_file_1cv7evt_txt; //  34h + 10h

	DWORD m_bufer1[0x4];      //  44h + 10h
	char  m_bufer2[0x8000];   //  44h + 8000h

	CFile m_file3;            //8054h + 10h
	DWORD m_data10;           //8064h
	DWORD m_data11;           //8068h
	DWORD m_data12;           //806Ch
	CString m_LogFileName;    //8070h

	//CLogDB**  m_ppMainLog;    //8074h
	CTableCSV** m_ppTableCSV; //8074h

	DWORD m_data14;           //8078h
	DWORD m_data15;           //807Ch
	DWORD m_data16;           //8080h
	DWORD m_data17;           //8084h
	DWORD m_data18;           //8088h
	DWORD m_data19;           //808Ch
	DWORD m_data20;           //8090h
	DWORD m_data21;           //8094h
	CString m_ModeName;       //8098h
	CString m_UserName;       //809Ch
	CString m_HostName;       //80A0h
	CString m_Path;           //80A4h
	DWORD m_data22;           //80A8h
	CString m_str6;           //80ACh

	CFile m_file4;            //80B0h + 10h
	CItemList m_ItemList;     //80C0h + 18h
	CFile m_file_links_tmp;   //80D8h + 10h

	DWORD m_data23;           //80E8h
	DWORD m_data24;           //80ECh
	DWORD m_data25;           //80F0h
	CString m_str7;           //80F4h
	//80F8h
...
class IMPORT_1C CUserCommandContaner : public CArray<CUserCommandInfo, CUserCommandInfo&>
{
public:
	//UserCommandInfoArr; // 0h + 14h
	CString m_Description;                          // 14h + 4h
	// 18H
public:
...
public:
	...
	DWORD m_mdType;
	DWORD m_mdID;
	CArray<CRightAtom, CRightAtom&> m_RightAtomArr;
...
class IMPORT_1C CRightsContainer : public CArray<CRightInfo, CRightInfo&>
{
public:
	//RightInfoArr; // 0h + 14h
	CString m_Description;                          // 14h + 4h
	DWORD m_data0;                                  // 18h + 4h
	BOOL m_IsInstalled;                             // 1Сh + 4h
	BOOL m_IsEnabled;                               // 20h + 4h
	CStringArray m_FullNameList;                    // 24h + 14h
	// 38h
...
//size of CApp7 is 8548h
class IMPORT_1C CApp7:public CAppFrame //CWinApp
{
public:
	CProfile7 m_Props;                              // 1B0h + 58h
	CUserCommandContaner m_UserCommandContaner;     // 208h + 18h
	CRightsContainer m_RightsContainer;             // 220h + 38h

	CArray<CCmdTarget*, CCmdTarget*> m_CmdTargetArr;// 258h + 14h
	DWORD m_ModeMask;                               // 26Ch + 4h

	CAdminService m_Log;                            // 270h + 80F8h

	CString m_UserSystemCaption;                    //8368h + 4h
	DWORD m_data1;                                  //836Ch + 4h
	DWORD m_ExitCode;                               //8370h + 4h

	COleObjectFactory m_InterfacePropPage;          //8374h + 4Ch
	//RTC = CInterfaceParamsPage

	COleObjectFactory m_TextPropPage;               //83C0h + 4Ch
	//RTC = CTextPage

	COleObjectFactory m_GeneralPropPage;            //840Ch + 4Ch
	//RTC = CGeneralParamsPage

	COleObjectFactory m_ModulePropPage;             //8458h + 4Ch
	//RTC = CModulePage

	COleObjectFactory m_AdminPropPage;              //84A4h + 4Ch
	//RTC = CAdminParamsPage

	COleObjectFactory m_SrcTemplatePropPage;        //84F0h + 4Ch
	//RTC = CSrcTemplatePage

	struct IzlibEngine * m_pzlibEngine;             //853Ch + 4h
	CWPIDoc* m_pWPIDoc;                             //8540h + 4h
	CRightDoc* m_pRightDoc;                         //8544h + 4h
...
{
	friend class CModuleWithDebugSupport;
public:
...
//<-CListViewEx(68h/1Ch)<-CListView(4Ch/0h)<-CCtrlView(4Ch/8h)<-CView(44h/4h)<-CWnd(40h/20h)<-CCmdTarget(20h/1Ch)<-CObject(4h/4h)
...
class IMPORT_1C CToolTip7 : public CToolTipCtrl
{
public:
	 CToolTip7(void);	//100
...
class IMPORT_1C CTopLevelMoxelContextDef : public CBLContext
{
public:
