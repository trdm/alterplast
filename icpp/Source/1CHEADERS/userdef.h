...
public:
	...
	virtual CString   GetDescription(void);	//65
protected:
...
public:
	...
	virtual void  SetDescription(CString);	//223
...
//Права
//<-CRightDoc(A4h/4h)<-CUsrDBaseDoc(A0h/14h)<-CContainer(8Ch/38h)<-CDocument(54h/34h)<-CCmdTarget(20h/1Ch)<-CObject(4h/4h)
...
public:
	...
	void  DescribeRights(CString &);	//49
...
public:
	...
	void  SetDescription(int,CString);	//224
protected:
...
public:
	...
	virtual CString   GetDescription(void);	//66
	virtual void  GetFilter(type_3 &,int);	//70
	virtual CString   GetFullName(CString);	//72
	void  GetIntFilter(type_3 &,class CUIntArray &,class CUserCommandContaner *);	//74
	CString   GetMasterName(void);	//76
	virtual class CMenu *  GetMenuShape(void);	//79
	class CUserInterface *  GetParentInterface(int);	//89
	int  GetResourse(class CUIntArray *,class CBitmap *,class CUIntArray *,CStringArray *);	//91
...
public:
	...
	virtual void  SetDescription(CString);	//225
	void  SetFilter(type_3 &);	//228
	void  SetMasterName(CString);	//229
...
public:
	...
	static CString   CommandToPresent(char const *);	//28
	virtual void  Create(class CWPIDoc *,char const *);	//34
	virtual int  DoFileSave(void);	//50
	int  GetCommandName(unsigned int,class CMenu *,CString &);	//59
	virtual class CUserCommandContaner *  GetContainer(void);	//63
	virtual CString   GetDescription(void);	//67
	virtual void  GetFilter(type_3 &,int);	//71
	virtual CString   GetFullName(CString);	//73
...
public:
	static CString   ParamToPresent(char const *);	//197
	static CString   PresentToCommand(char const *);	//201
	static CString   PresentToParam(char const *);	//202
	virtual void  Rename(char const *);	//207
protected:
	virtual int  SaveModified(void);	//214
public:
	void  SetDefault(int);	//221
	virtual void  SetDescription(CString);	//226
...
//Интерфейсы
//<-CWPIDoc(A4h/4h)<-CUsrDBaseDoc(A0h/14h)<-CContainer(8Ch/38h)<-CDocument(54h/34h)<-CCmdTarget(20h/1Ch)<-CObject(4h/4h)
...
//<-CUsrDBaseDoc(A0h/14h)<-CContainer(8Ch/38h)<-CDocument(54h/34h)<-CCmdTarget(20h/1Ch)<-CObject(4h/4h)
class CUsrDBaseDoc :public CContainer
{
public:
	CString m_strDocTitle; // 8Ch
	DWORD m_nUsersCount;   // 90h
	DWORD m_data1;         // 94h
	DWORD m_data2;         // 98h
	CString m_strPathName; // 9Ch
	                       // A0h
};

//<-CUsersDoc(A4h/4h)<-CUsrDBaseDoc(A0h/14h)<-CContainer(8Ch/38h)<-CDocument(54h/34h)<-CCmdTarget(20h/1Ch)<-CObject(4h/4h)
class CUsersDoc :public CUsrDBaseDoc
{
public:
	CString m_str; // A0h
	               // A4h
...
//<-CUsrDBaseForm(84h/1Ch)<-CListViewEx(68h/1Ch)<-CListView(4Ch/0h)<-CCtrlView(4Ch/8h)<-CView(44h/4h)<-CWnd(40h/20h)<-CCmdTarget(20h/1Ch)<-CObject(4h/4h)
class CUsrDBaseForm : public CListViewEx
{
public:
...
//<-CUsrListView(88h/4h)<-CUsrDBaseForm(84h/1Ch)<-CListViewEx(68h/1Ch)<-CListView(4Ch/0h)<-CCtrlView(4Ch/8h)<-CView(44h/4h)<-CWnd(40h/20h)<-CCmdTarget(20h/1Ch)<-CObject(4h/4h)
class CUsrListView : public CUsrDBaseForm
{
public:
	CString m_strUnkHash;  //84h
	                       //88h
};
