...
public:
	...
	virtual int				GetParamDefValue(int nMethIndex, int nParamIndex, CValue* pDefValue)const
    {
        switch (nMethIndex)
        {
        case methGet:
	...
	virtual int				CallAsProc(int nMethIndex, CValue** ppValue)
    {
        if (dwFlags & BLC_MODIFED)
        {
            switch (nMethIndex)
            ...
            case methRemove:
				{
					...
					{
						case NUMBER_TYPE_1C:
							nIndex = ppValue[0]->GetNumeric();
							break;
						case STRING_TYPE_1C:
							nIndex = IndexOf(ppValue[0]->GetString());
							break;
						default:
	...
	virtual int				CallAsFunc(int nMethIndex, CValue& RetValue, CValue** ppValue)
    {
        ...
        case methAdd:
            if (dwFlags & BLC_MODIFED)
                RetValue.AssignContext(static_cast<ItemType*>(Add(ppValue[0]->GetString())));
            break;
        case methInsert:
            if (dwFlags & BLC_INSERT)
            {
...
template <class CollType, class ItemType, CRuntimeClass* pItemRuntimeClass, DWORD dwFlags>
CParamDefs CBLCollection<CollType, ItemType, pItemRuntimeClass, dwFlags>::m_ParamDefs;
