...
CBinaryData::CBinaryData()
{
  ...
  {     
    ...
    defFnNames.AddParam("pkZip", "pkZip", procPkZip, 0, 1);
    defFnNames.AddParam("pkUnzip", "pkUnzip", procPkUnzip, 0, 1);
    defFnNames.AddParam("pkCompress", "pkCompress", procPkCompress, 0, 1);
    defFnNames.AddParam("pkDecompress", "pkDecompress", procPkDecompress, 0, 1);
    defFnNames.AddParam("pkAppendFile", "pkAppendFile", procPkAppendFile, 0, 3);
    defFnNames.AddParam("pkReadFile", "pkReadFile", procPkReadFile, 0, 3);
    defFnNames.AddParam("pkReadEntry", "pkReadEntry", procPkReadEntry, 0, 2);
  }
  m_Encoding = ANSI;
}
...
int  CBinaryData::CallAsFunc(int iMethNum,CValue & rValue,CValue * *ppValue)
{
    int ret = 0;
	switch(iMethNum)
  {
...
int  CBinaryData::CallAsProc(int iMethNum,CValue * * ppValue)
{
    int ret = 0;
	switch(iMethNum)
  {
    case procWriteString:
      {
          ...
          const void* pData;
          ULONG nLength;
          sh_array<WCHAR> pWStr;
          sh_array<CHAR> pUtfStr;
          ...
          case OEM:
              ...
              break;
          case UTF_16:
              pWStr = sh_array<WCHAR>(new WCHAR[sData.GetLength()]);
          ...
          case UTF_8:
              ...
              pUtfStr = sh_array<CHAR>(new char[nLength]);
    ...
    case procZip:
    case procPkZip:
		//ppValue[0] - CommandLine
      {
        CComPtr<IzlibEngine> Zlib;
		HRESULT hres;
		hres = Zlib.CoCreateInstance(__uuidof(zlibEngine));
		AssertHResult(hres);

		hres = Zlib->pkZip(
			(LPWSTR)ppValue[0]->GetString().operator LPCSTR()
			);
		AssertHResult(hres);
      }
      break;
    ...
    case procPkReadEntry:
		...
		//ppValue[1] - ValueList
      {
		...
		IEnum_zipEntry* pEnumEntry = NULL;
		hres = Zlib->pkReadEntry(
			(LPWSTR)ppValue[0]->GetString().operator LPCSTR(),
			&pEnumEntry
			);
		AssertHResult(hres);

		CValueItemList* pVL = CValue2VL(*ppValue[1]);
		if (pVL)
		{
			#define PkReadEntry_MAX_ITEM_COUNT 20
			IzipEntry* pzipEntrys[PkReadEntry_MAX_ITEM_COUNT];

			ULONG nFitched;
			do 
			{
				pEnumEntry->Next(PkReadEntry_MAX_ITEM_COUNT , pzipEntrys, &nFitched);
				for (int i=0; i<nFitched; i++)
				{
					//!!!!!
					//Порядок вызовов не менять
					CStdOleBLContext* pOleCont = (CStdOleBLContext*)RUNTIME_CLASS(CStdOleBLContext)->CreateObject();
					pOleCont->m_OleDispatchDriver.AttachDispatch(pzipEntrys[i]);
					pOleCont->TryFindFriendInterface();

					CValueItem* pValueItem = new CValueItem;
					pValueItem->m_value.AssignContext(pOleCont);
					pOleCont->DecrRef();
					//!!!!!
    ...
    case procPkCompress:
		//ppValue[0] - BinaryDataTo
      {
		...
		CBinaryData* pBinDataCont = (CBinaryData*)ppValue[0]->GetContext();
		if (pBinDataCont)
		{
			if (pBinDataCont->IsKindOf(RUNTIME_CLASS(CBinaryData)))
			{
				hres = Zlib->pkCompress(GetIStream(), pBinDataCont->GetIStream());
				AssertHResult(hres);
			}
		}
      }
      break;
    case procPkDecompress:
		//ppValue[0] - BinaryDataTo
      {
        CComPtr<IzlibEngine> Zlib;
		HRESULT hres;
		...
		if (pBinDataCont)
		{
			if (pBinDataCont->IsKindOf(RUNTIME_CLASS(CBinaryData)))
			{
				hres = Zlib->pkDecompress(GetIStream(), pBinDataCont->GetIStream());
				AssertHResult(hres);
			}
		}
      }
      break;
    case procPkAppendFile:
		//ppValue[0] - FileTo
		//ppValue[1] - StreamName
		//ppValue[2] - Password
      {
        CComPtr<IzlibEngine> Zlib;
		HRESULT hres;
		...
		const CString& Password = ppValue[2]->GetString();
		if (!Password.IsEmpty())
		{
			pszPassword = (LPWSTR)Password.operator LPCSTR();
		}

		hres = Zlib->pkAppendFile(
			GetIStream(),
			(LPWSTR)ppValue[0]->GetString().operator LPCSTR(),
			(LPWSTR)ppValue[1]->GetString().operator LPCSTR(),
			pszPassword
			);
		AssertHResult(hres);
      }
    ...
    case procPkReadFile:
		...
		//ppValue[2] - Password
      {
		...
		hres = Zlib->pkReadFile(
			(LPWSTR)ppValue[0]->GetString().operator LPCSTR(),
			(LPWSTR)ppValue[1]->GetString().operator LPCSTR(),
			GetIStream(),
			(LPWSTR)ppValue[2]->GetString().operator LPCSTR()
			);
		AssertHResult(hres);
      }
      break;
...
int  CBinaryData::GetParamDefValue(int iMethodNum,int iParamNum,CValue * pDefValue)const
{	
	switch(iMethodNum)
  {
    case funcLoadFromFile:
...
void  CBinaryData::GetExactValue(CValue & vParam)
{
...
CType   CBinaryData::GetValueType(void)const
{
	CType tType(100);	
	return tType;
}
...
//void  CBinaryData::InitObject(CType const & tType)
//{
...
int  CBinaryData::SaveToString(CString & csStr)
{	
	return 0;
}
...
int  CBinaryData::GetPropVal(int iPropNum,CValue & rValue)const
{
    rValue = m_Encoding;
	return 1;
}

int  CBinaryData::SetPropVal(int iPropNum,CValue const & vValue)
{
...
{
    return (LPSTR)strstr(pStr, "\r\n");
};

LPWSTR FindTerm(LPCWSTR pStr)
{
    return (LPWSTR)wcsstr(pStr, L"\r\n");
};
