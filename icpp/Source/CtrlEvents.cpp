                                  ...
                                  CBLContext* pDerivedObject)
{
	...
	for (; nCount > 0; nCount--, pCtrlEvents++)
	{
		...
		if (event.nFormEvent != -1)
		{
			...
			if (BLProcInfo.GetNParams() > pCtrlEvents->nParamCount+1)
			{
				...
				str.Format("Количество параметров в событии '%s' должно быть меньше или равно %u.",
					strProcName.operator LPCTSTR(), pCtrlEvents->nParamCount+1);
		...
		if (pDerivedObject)
		{
			...
			if (event.nObjEvent != -1)
			{
				...
				{
					CString str;
					str.Format("Количество параметров в событии '%s::%s'  должно быть меньше или равно %u.",
	...
	if (!bSuccess)
		RuntimeError("Ошибка подключения событий.");
}
...
BOOL CCtrlEventManager::Invoke5(int nEventIndex, CValue& vP1, CValue& vP2, CValue& vP3, CValue& vP4, CValue& vP5)
{
	CValue vCtrl;
	vCtrl.AssignContext(m_pCtrlObject);
	CValue* ppValues[] = {&vCtrl, &vP1, &vP2, &vP3, &vP4, &vP5};
	return InvokeEvent(nEventIndex, ppValues);
}
...
{
    BOOL bSuccess = TRUE;
    stEvent const& event = m_events[nEventIndex];
	stCtrlEvents const& ctrlEvent = m_pCtrlEvents[nEventIndex];
	...
	{
		LogErr("Исключение при обработке события %s()!", ctrlEvent.szRusName);
	}

	if (event.nFormEvent != -1)
		bSuccess = bSuccess && m_pModule->CallAsProc(event.nFormEvent, ctrlEvent.nParamCount + 1, ppValuesParams);
