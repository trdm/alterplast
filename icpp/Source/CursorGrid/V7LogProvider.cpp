...
BOOL CLOG_DP_LogFile::ReadArchive(const POSITION& PositionInList)
{
	...
	else
	{
		...
		std::vector<char> pBufVector(nBufferSize); //void* pBuf=malloc(nBufferSize);
...
BOOL CV7LogProvider::CreateTempLog(const CString& csBegDate, const CString& csEndDate, ULONGLONG& nOffset)
{
	...
	if (hFind!=INVALID_HANDLE_VALUE)
	{
		...
		{
			RuntimeError(_T("Недостаточно места: %s"), csPath.operator LPCTSTR());
			return FALSE;
		}
...
BOOL CV7LogProvider::SetPeriod(CValue& Result, CValue** ppValue)
{
	...
	{
		CString strBeginOfDay( ppValue[0]->GetString()+_T(";00:00:00;") );
		CString strEndOfDay( ppValue[1]->GetString()+_T(";23:59:59;") );
		if (SetPeriodInternal(strBeginOfDay, strEndOfDay))
			Result=CNumeric(1);
	}
...
BOOL CV7LogProvider::ChangeFilter(CValue& Result, CValue** ppValue)
{
	...
	else
	{
		...
		case AGREGATE_TYPE_1C:
			...
			{
				if (CValueItemList* pVL=CValue2VL(*ppValue[1]))
				{
					for (int i=0; i<pVL->GetSize(); i++)
					{
						CValue* pVal=&pVL->GetAt(i)->m_value;
	...
	if (m_bLinesSelectionEnabled)
	{
		...
		else
		{
			if (!m_sCurrentRow.Fields[_ROW_FIELD_INDEX_(nFieldIndex)].IsEmpty())
			{
				...
				case LOG_DP_FILTER_OBJECT:
					{
						CString str = CString(m_sCurrentRow.Fields[_ROW_FIELD_INDEX_(nFieldIndex)]);
						LogValueFromString(Value, str, 1);
					}
					break;
				case LOG_DP_FILTER_EVENTTYPE:
