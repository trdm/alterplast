...
BOOL CV7SelectListItem::SetPropType(CValue const& Value)
{
	...
	{
		RuntimeError("Недопустимая типизация для поля %s.", strName.operator LPCTSTR());
		return FALSE;
	}
    return TRUE;
}
...
void CV7ODBCDataRow::SQLFormatValue(const CValue &value, CString& strValue, CV7ODBCSQLParam const* pParam)
{
    bool bIsStrLiteral;
    ...
    switch (nTypeCode)
    {
	case UNDEFINED_TYPE_1C:
		nModificator = UNDEF_TYPE_MODIFIKATOR;
		break;
    case DATE_TYPE_1C:
        nModificator = 2;
        break;
	default:
		if (pParam && nTypeCode >= ENUM_TYPE_1C && nTypeCode <= CHART_OF_ACC_TYPE_1C)
		{
			switch (pParam->m_nPrecision)
			{
			case 13:
				nModificator = 1;
				break;
...
void CV7ODBCSQLParam::FormatValue(CString& strValue)
{
	...
	case NUMBER_TYPE_1C:
		if (m_nSQLType == SQL_CHAR && m_nPrecision == 23)
			strValue.Format("'N%22s'", m_pValue->GetString().operator LPCTSTR());
		else if (m_nSQLType == SQL_CHAR || m_nSQLType == SQL_VARCHAR)
			strValue.Format("'%s'", m_pValue->GetString().operator LPCTSTR());
		else
			strValue = m_pValue->GetString();
		break;
	case STRING_TYPE_1C:
		if (m_nSQLType == SQL_CHAR && m_nPrecision == 23 && m_pValue->GetString().GetLength() == 22)
			strValue.Format("'S%-22s'", m_pValue->GetString().operator LPCTSTR());
		else if (m_nSQLType == SQL_CHAR || m_nSQLType == SQL_VARCHAR)
			strValue.Format("'%s'", m_pValue->GetString().operator LPCTSTR());
		else
			strValue = m_pValue->GetString();
	...
	case ACCOUNT_TYPE_1C:
		...
		{
			if (m_nPrecision == 9)
				strValue.Format("'%s'", CMetaDataWork::GetObjDBString(*m_pValue, CMetaDataWork::ShortString).operator LPCTSTR());
			else if (m_nPrecision == 13)
				strValue.Format("'%s'", CMetaDataWork::GetObjDBString(*m_pValue, CMetaDataWork::LongString).operator LPCTSTR());
			else if (m_nPrecision == 23)
				strValue.Format("'%s'", CMetaDataWork::GetObjDBString(*m_pValue, CMetaDataWork::VeryLongString).operator LPCTSTR());
		}
		break;
	...
	if (!m_params.IsEmpty())
	{
		...
		if (m_pProvider->GetQueryStyle() == qsMSSQL)
		{
			...
			while(pos)
			{
				...
				if (pParam->m_bIsOK)
				{
					pRowFrom->SQLFormatValue(*pRowFrom->m_values[pParam->m_nFieldIndex], strTmp, pParam);
		...
		else
		{
			...
			while(pos)
			{
				...
				if (!pParam->m_bIsOK)
				{
					CV7ODBCDataRow::SQLFormatValue(*pParam->m_pValue, strTmp, pParam);
    ...
    if (*pSelectListItem->GetName() == '[')
    {
        ...
        if (pDataBase7->GetDataSourceType() == DATA_SOURCE_TYPE_DBF)
            if (strName.Replace(TypeNamePrefix, "$"))
                strName.Replace(szDotSubst, ".");

        if (!CODBCRecordset::ParseV7FieldType(strName, pItem->m_rt, pItem->m_AddType, pItem->m_BLCTypeName))
        {
            m_strLastError.Format("Недопустимая типизация для поля %s.", strName.operator LPCTSTR());
            return FALSE;
        }
...
CSQLQueryParam* CV7ODBCProvider::NewSQLQueryParam(const CString& strName, const CStringList& params, CString& strLabel)
{
    ...
    if (!pos)
    {
        m_strLastError.Format("Не указан тип параметра \"%s\"", strName.operator LPCTSTR());
        return NULL;
    }
    ...
    if (pos)
    {
        ...
        if (pos)
        {
            ...
            if (pos)
            {
                m_strLastError.Format("Не допустимое задание параметра \"%s\"", strName.operator LPCTSTR());
                return NULL;
            }
        }
    }
    ...
    default:
        m_strLastError.Format("Не допустимый тип параметра \"%s\"", strName.operator LPCTSTR());
        return NULL;
    }
	...
	pParam->m_strLabel.Format("@QParam_%s_%d_%d_%d", strName.operator LPCTSTR(), nSQLType, nPrecision, nScale);
...
void CV7ODBCProvider::QuickSearch(stQuickSearchPattern& QSPattern)
{
    ...
    while (nBegin != nEnd && nBegin + nCount)
    {
		...
		if (m_bDebug)
			Msg("%s", strQTCopy.operator LPCTSTR());
