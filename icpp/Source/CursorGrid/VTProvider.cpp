...
{
	if (nIndex == 0) //це НомерСтроки
		return dtNumeric;

	switch (m_pVT->GetColumn(nIndex - 1)->GetType().GetTypeCode())
	{
	case NUMBER_TYPE_1C:
		return dtNumeric;
	case DATE_TYPE_1C:
...
BEGIN_BL_PROP_MAP(CVTProvider)
	BL_PROP("ValueTable", "ТаблицаЗначений", &GetPropValueTable, &SetPropValueTable)
END_BL_PROP_MAP()
...
BOOL CVTProvider::GetPropValueTable(CValue& Value) const
{
	Value.AssignContext(m_pBLVT);
	return TRUE;
}

BOOL CVTProvider::SetPropValueTable(CValue const& Value)
{
	CValue* ppValue = const_cast<CValue*>(&Value);
	return MethSetValueTable(&ppValue);
}
