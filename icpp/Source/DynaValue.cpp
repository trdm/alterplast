...
int CDynaValue::GetSelectedProp(CValue *rProp, CValue *rName)
{
	rName->Reset();
	rProp->Reset();
...
int  CDynaValue::CallAsFunc(int iMethNum,CValue & rValue,CValue * *ppValue)
{
	int rc=1;
...
int  CDynaValue::CallAsProc(int iMethNum,CValue * * ppValue)
{
	int rc=1;
	int i;
...
int  CDynaValue::GetParamDefValue(int iMethodNum,int iParamNum,CValue * pDefValue)const
{
	return 0;
}
...
void  CDynaValue::GetExactValue(CValue & vParam)
{
...
CType   CDynaValue::GetValueType(void)const
{
	return CType(100);
}
...
void  CDynaValue::InitObject(CType const & tType)
{
...
int  CDynaValue::SaveToString(CString & csStr)
{
#ifdef DBG_DV_SAVE
	...
	if(npr)
	{
		while(pd)
		{
			if((pd->flags & IFLAG_RUNTIME)==0)
			{
				...
				if(n = IsDMSContext(type) || (ISetOfHierarchy::GetMetaDataOfClasses().IsClassExist(type)&&pd->val->GetContext()->IsSerializable()))
				{
...
int  CDynaValue::GetPropVal(int iPropNum,CValue & rValue)const
{
	CValue *val;
...
int  CDynaValue::SetPropVal(int iPropNum,CValue const & vValue)
{
	CValue *val;

	if(val = FindPropVal(iPropNum))
	{
		if (vValue.GetTypeCode() == UNDEFINED_TYPE_1C)
			val->Reset();
		else
