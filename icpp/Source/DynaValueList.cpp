...
void CDynaValueList::DeleteValues()
{
	...
	while(pd)
	{
		if(pd->val)
		{
			*pd->val = 0L;
			m_fNode->Free(pd->val);
			m_fNode->UnRegAlloc(sizeof(CValue));
		}
		pd->val=0;
		next=pd->next;
		m_fNode->Free(pd);
...
//			realsz = fsz>256?fsz:256;
			...
			if(mem)
			{
				...
				((byte*)mem)[fsz-1]=0;
				npd->val = new CValue();
				m_fNode->RegAlloc(sizeof(CValue));
				if(lastProp==0)
				{
					lastProp=npd;
					firstProp=npd;
		...
		{
			sprintf(buf,"—войство с именем %s уже существует", name.operator LPCTSTR());
...
int CDynaValueList::AddValue(const CString &name, const CString &valname, CValue *defVal, int prop_flags)
{
	...
	if(sz>0)
	{
		if(pv = findNpd(firstVal,name))
		{
			...
			{
				sprintf(buf,"«начение с именем '%s' уже существует", name.operator LPCTSTR());
				CBLModule::RaiseExtRuntimeError(buf,0);
				return 0;
			}
		}
...
int CDynaValueList::GetSelectedProp(CValue *rProp, CValue *rName)
{
	rName->Reset();
	rProp->Reset();
...
int CDynaValueList::GetSelectedValue(CValue *rProp, CValue *rName)
{
	rName->Reset();
	rProp->Reset();
...
int  CDynaValueList::CallAsFunc(int iMethNum,CValue & rValue,CValue * *ppValue)
{
	int i;
	int rc = 1;
...
//	iMethNum = m_fNode->m_license->CheckExpiration(iMethNum);
	...
	case methAddProp:
	case methAddPropA:
		rc = AddProp(ppValue[0]->GetString(),ppValue[1],iMethNum==methAddPropA ? (int)ppValue[2]->GetNumeric() : 0);
		if(rc)
		{
...
int  CDynaValueList::CallAsProc(int iMethNum,CValue * * ppValue)
{
	int rc=1;
	int i;
...
int  CDynaValueList::GetParamDefValue(int iMethodNum,int iParamNum,CValue * pDefValue)const
{
	return 0;
}
...
void  CDynaValueList::GetExactValue(CValue & vParam)
{
...
CType   CDynaValueList::GetValueType(void)const
{
	return CType(100);
}
...
void  CDynaValueList::InitObject(CType const & tType)
{
...
int  CDynaValueList::SaveToString(CString & csStr)
{
#ifdef DBG_DVL_SAVE
...
int  CDynaValueList::GetPropVal(int iPropNum,CValue & rValue)const
{
	CValue a[2];
...
int  CDynaValueList::SetPropVal(int iPropNum,CValue const & vValue)
{
	CValue *val;
	const char *type;
...
int  CDynaValueList::FindProp(char const * Name) const
{
	long i = 0;
	struct propdefs *pd = firstProp;
