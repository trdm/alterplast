...
//	BL_METH_PROC("ТестОтключитьБыстроеСоздание_Артур", "ТестОтключитьБыстроеСоздание_Артур", 1, &procDisableFastCreationAllContexts)
//#endif
...
BOOL CObjectFactory::funcNewObject(int nParamsCount, CValue &RetVal, CValue **ppValue)
{
	...
	if (!pCont)
		RuntimeError("Фабрика.Новый - Ошибка при создании класса %s", strTypeName.operator LPCTSTR());
	...
	if (nParamsCount >= 2)
	{
		...
		else
		{
			...
			if (-1 == iProc)
				RuntimeError("У класса <%s> не существует процедуры-конструктора <%s>", strTypeName.operator LPCTSTR(), strConstructorName.operator LPCTSTR());

			if (pCont->HasRetVal(iProc))
				RuntimeError("У класса <%s> не существует процедуры-конструктора <%s>, но есть такая функция", strTypeName.operator LPCTSTR(), strConstructorName.operator LPCTSTR());
			...
			if(nMethodParams < nCallParams)
				RuntimeError("%s::%s - Слишком много фактических параметров при вызове", strTypeName.operator LPCTSTR(), strConstructorName.operator LPCTSTR());
			...
			{
				if((*ppValueForCallMethod)->type == -1)	// Значение по умолчанию // TODO беззнаковое значение сравнивается с -1 :(
				{
					if(!pCont->GetParamDefValue(iProc, i, *ppValueForCallMethod))
						RuntimeError("%s::%s - Неуказан обязательный параметр № %i", strTypeName.operator LPCTSTR(), strConstructorName.operator LPCTSTR(), i);
				}
