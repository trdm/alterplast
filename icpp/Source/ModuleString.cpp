...
bool CModuleString::LoadModuleFromERT(const CString& strFileName)
{
	...
	try
	{
		...
		CApp7* pApp = static_cast<CApp7*>(AfxGetApp());
		if (pApp)
...
bool CModuleString::LoadModuleFromMD(const CString& strFileName)
{
	...
	if (pObj)
	{
		...
		if(pCont)
		{
			CTxtModuleReader* pText = static_cast<CTxtModuleReader*>(pCont->GetTextDocument());
			pText->GetText(*this);
...
bool CModuleString::LoadModuleFromFile(const CString& strFileName)
{
	...
	//CString tempstr;
	//if(ReadFileToStringWithoutExceptions(strFileName, tempstr))
	{
		//static_cast<CString&>(*this) = tempstr;
