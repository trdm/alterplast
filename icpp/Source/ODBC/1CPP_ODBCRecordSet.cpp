    ...
    case funcExecuteStatement:
      {
		...
		CValueTable* pVT;
		CValueListContextFriend* pVLC;
		CVTExtended* pVTExt;
		CValue const& val = *ppValue[1];
		...
		{
			(pVT = CValue2VT(val)) || (pVLC = CValue2VLC(val)) || (pVTExt = CValue2VTExt(val));
			rValue = val;
		}
		...
		{
            if (ppValue[2]->GetNumeric().operator long())
                pVLC->P_RemoveAll(NULL);

            if (GetFieldCount())
			{
				CValueItemList* pVL = pVLC->GetPValueList();
    ...
    case funcExeSQL_FromTV:
      {
        ...
        if (m_IsDebugMode)
        {
          ...
          {
            ISetOfHierarchy *pH = ISetOfHierarchy::GetHierarchy();
            ...
            case STRING_TYPE_1C:
              {
                ...
                if (dest)
                {
                  ...
                  {         
                    ...
                    CString s(token);
                    s.Replace(" ", "");
                    int pos = GetFieldIndex(s);
                    if (pos == -1)
                    {
                      CString err;
                      err.Format("Данной колонки с именем: %s не существует в выборке!", s.operator LPCTSTR());
                      free(dest);
                      CBLModule::RaiseExtRuntimeError(err, 0);
                    }
    ...
    case funcGetValue:
      {
        ...
        else if(ppValue[0]->GetTypeCode() == STRING_TYPE_1C)
        {
          ...
          {
            CString err;
            err.Format("Колонка с именем (%s) отсутсвует в выборке!", ppValue[0]->GetString().operator LPCTSTR());
            CBLModule::RaiseExtRuntimeError(err, 0);
          }
        }
        else
    ...
    case funcGetInTablesValues:
      {
        ...
        else
        {
          ...
          else
          {
            int staticColumnCount = pCont->FindMethod("ColumnCount");
            pCont->CallAsFunc(staticColumnCount,arValue, ppPs);
            n = min(GetFieldCount(),(int)arValue.GetNumeric());            
          }
    ...
    case procPutObjectList13:
      {
        ...
        CString Query = "if exists(select * from tempdb..sysobjects where id = object_id('tempdb..%tmp'))\r\n\
            drop table %tmp\r\n\
            create table %tmp (val char(13) collate database_default, primary key clustered (val))";
        Query.Replace("%tmp", TableName);
        Execute(Query);

        Query.Format("insert %s values (?)", TableName.operator LPCTSTR());
        Prepare(Query);
        ...
        if (ppValue[0]->GetTypeCode() == AGREGATE_TYPE_1C)
        {
            ...
            if (pListCtx && !strcmp(pListCtx->GetRuntimeClass()->m_lpszClassName,"CValueListContext"))
            {
                ...
                for(int i=0;i<listSize;i++)
                {
                    ...
                    static_cast<CDBField&>(qp) = m_pMetaData->GetObjDBString(*pItem, CMetaDataWork::LongString);
                    Execute(NULL, TRUE);
                }
            }
        } 
        ...
        {
            static_cast<CDBField&>(qp) = m_pMetaData->GetObjDBString(*ppValue[0], CMetaDataWork::LongString);
            Execute(NULL, TRUE);
        }
        else
        {
  ...
  case procPutObjectList:
      {
        ...
        Query.Format("insert %s values (?, 2)", TableName.operator LPCTSTR());
        Prepare(Query);
      ...
      if (ppValue[0]->GetTypeCode() == AGREGATE_TYPE_1C)
      {
          ...
          if (pCont && !strcmp(pCont->GetRuntimeClass()->m_lpszClassName,"CValueListContext"))
          {
            ...
            {
	            CValue* pItem=(CValue*)pVL->GetAt(i);
                static_cast<CDBField&>(qp) = m_pMetaData->GetObjDBString(*pItem, CMetaDataWork::ShortString);
                Execute(NULL, TRUE);
            }
            bOK = TRUE;
          }
      ...
      {
        static_cast<CDBField&>(qp) = m_pMetaData->GetObjDBString(*ppValue[0], CMetaDataWork::ShortString);
        Execute(NULL, TRUE);
        bOK = TRUE;
      }
...
BOOL C1CPP_ODBCRecordSet::TypifyField(int nIndex, CDBField& field)
{
    if (pDataBase7->GetDataSourceType() == DATA_SOURCE_TYPE_DBF)
        if (field.m_strName.Replace(TypeNamePrefix, "$"))
            field.m_strName.Replace(szDotSubst, ".");
