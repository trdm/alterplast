...
enum mnSubTypes {mnTable, mnField, mnId, mnSpecial, mnVirtualTable, mnVirtualValue};
...
bool CMetaNameParser::CValueToDBValue(const CValue & Value, const int Modificator, CString & StrValue, bool & IsStrLiteral, CString& SQLType)
{
	...
	case MAKELONG(NUMBER_TYPE_1C, UNDEF_TYPE_MODIFIKATOR):
		StrValue.Format("N%22s", Value.GetString().operator LPCTSTR());
		break;
	...
	case MAKELONG(STRING_TYPE_1C, UNDEF_TYPE_MODIFIKATOR):
		StrValue.Format("S%-22s", Value.GetString().operator LPCTSTR());
		break;
	...
	case MAKELONG(STRING_TYPE_1C, 1):
		try
		{
			...
			IdBuff[6] = 0;
			StrValue.Format("%8s%6s%6s%3s", Pos.Mid(1, 8).operator LPCTSTR(), TimeBuff, IdBuff, Pos.Right(3).operator LPCTSTR());
			SQLType = "char(23)";
		}
		catch (...)
		{
...
void CMetaNameParser::Parse()
{
	...
	while (GetNextSource(BeginParamSimbol))
	{
		...
		if (Params.Lookup(Param, Value))
		{
			...
			{
				CString cs;
				PasteName(CValueToSQLLiteral(Value, Modifikator, cs));
			};
		}
		else
...
case ntRegistrRestsTurnovers:
	...
	if (TmpAlias.IsEmpty()) {
		...
		Name.Format("(\r\n%s) as %s", SubQuery.operator LPCTSTR(), Alias.operator LPCTSTR());
	};
					 }
					 break;
case mnVirtualValue: {
...
void CMetaNameParser::RaiseException(const char * DescTemplate)
{
	...
	CString ErrDesc;
	Start--;
	ErrDesc.Format(DescTemplate, QueryText.Mid(Start, CurIndex - Start).operator LPCTSTR());
	QueryText.Empty();
	...
	if (!Joines.IsEmpty() || !Conditions.IsEmpty())
	{
		...
		} catch (CMNPException * MNPException)
		{
			if (MNPException->GetCurrentIndex() > QueryText.Find(Separator))
				NParam++;
			CString Str;
			Str.Format("(%d : %s)", NParam, MNPException->GetErrorDescr().operator LPCTSTR());
			MNPException->Delete();
			...
			if (IsIdChar(CurChar))
			{
				...
				if (!fDotOrDog)
				{
					for (int i = 0; i < ArraySize; i++)
					{
						...
						if (Obj)
						{
							Id.Format("%s.sp%d", m_sTempFieldPrefix.operator LPCTSTR(), Obj->GetID());
...
CString CMetaNameParser::GetStringDateTimeIddoc(const CDate & Date, const long Time, const CObjID & ObjID)
{
	...
	IDBuff[6] = 0;
	CString s;
	s.Format("%s%6s%6s%3s", CDateToCString(Date).operator LPCTSTR(), TimeBuff, IDBuff, ObjID.DBSign.Sign);
	return s;
}
	...
	if (RestsPlan.QType != rqNoTurnovers) {
		...
		if (RestsPlan.QType == rqForward) {
			...
			if (GetBegOfPeriod(DateRests, RestsSnapShotPeriod).m_DateNum == GetBegOfPeriod(DateTA, RestsSnapShotPeriod).m_DateNum)
				...
				RestsPlan.TurnEnd.Format("%sZ", CDateToCString(GetEndOfPeriod(DateRests, RestsSnapShotPeriod)).operator LPCTSTR());
...
void CMetaNameParser::BuildRegRestsSubQuery(CRegDef* RegDef, const PMetaDataObjArray & Dims, const PMetaDataObjArray & Figs, const CString & Joines, const CString & Conditions, const stRestsPlan & RestsPlan, CString & SubQuery)
{
	...
	if (RestsPlan.QType == rqNoTurnovers)
	{
		if (Dims.GetSize() == RegDef->GetProps()->GetNItems())
		{
			...
			TmpStr.Format("from rg%d as %s (nolock)\r\n", RegDef->GetID(), sTotName.operator LPCTSTR()); SubQuery += TmpStr;

			AddSubToken(SubQuery, Joines, 'g', arJoinesPS);

			TmpStr.Format("where %s.period=%s\r\n", sTotName.operator LPCTSTR(), sRestsPeriod.operator LPCTSTR()); SubQuery += TmpStr;
	...
	else
	{
		...
		if (RestsPlan.QType == rqBack)
			nSign = 1;
		TmpStr.Format("case %s.debkred when %d then %s.%%id else -%s.%%id end", sTurnName.operator LPCTSTR(), nSign, sTurnName.operator LPCTSTR(), sTurnName.operator LPCTSTR());

		AddMDObjArray(SubQuery, Figs, RestsPlan.EnlargeStr + TmpStr, ",");
		TmpStr.Format("from ra%d as %s (nolock)\r\n", RegDef->GetID(), sTurnName.operator LPCTSTR()); SubQuery += TmpStr;
		CString sDateTimeName;
		if (!RegDef->GetDateTimeInActs())
		{
			sDateTimeName.Format("j%d_vt", RegDef->GetID());
			TmpStr.Format("inner join _1sjourn as %s (nolock) on %s.iddoc = %s.iddoc\r\n", sDateTimeName.operator LPCTSTR(), sDateTimeName.operator LPCTSTR(), sTurnName.operator LPCTSTR()); SubQuery += TmpStr;
		}
		else
		...
		if (RestsPlan.QType == rqBack)
			SubQuery += '=';
		TmpStr.Format(" %s and %s.date_time_iddoc < %s\r\n", GetVTStringParam(RestsPlan.TurnBegin).operator LPCTSTR(), sDateTimeName.operator LPCTSTR(), GetVTStringParam(RestsPlan.TurnEnd).operator LPCTSTR()); SubQuery += TmpStr;
		if (!RegDef->GetDateTimeInActs())
		{
			TmpStr.Format("and %s.rf%d = 0x1\r\n", sDateTimeName.operator LPCTSTR(), RegDef->GetID()); SubQuery += TmpStr;
		};
		AddSubToken(SubQuery, Conditions, 'a', arConditionsPS);

		if (RestsPlan.EnlargeStr.IsEmpty())
		{
			TmpStr.Format(") as %s\r\n", sVTName.operator LPCTSTR()); SubQuery += TmpStr;
			SubQuery += "group by\r\n";
...
case eTenDays:
	if (SourceType)
	Str = "case when day(%s)<=10 then dateadd(dd,-day(%s)+1,%s) \r\n\
	       when day(%s)<=20 then dateadd(dd,-day(%s)+11,%s) \r\n\
	       else dateadd(dd,-day(%s)+21,%s) \r\n\
	       end \r\n";
	else
	Str = "case when day(cast(left(%s, 8) as datetime))<=10 then dateadd(dd,-day(cast(left(%s, 8) as datetime))+1,cast(left(%s, 8) as datetime)) \r\n\
	       when day(cast(left(%s, 8) as datetime))<=20 then dateadd(dd,-day(cast(left(%s, 8) as datetime))+11,cast(left(%s, 8) as datetime)) \r\n\
	       else dateadd(dd,-day(cast(left(%s, 8) as datetime))+21,cast(left(%s, 8) as datetime)) \r\n\
	       end \r\n";
	break;
case eMonth:
	if (SourceType)
...
case eYear:
	...
	if (BegLim.GetYear() && Periodcity != eDay) {
		CString s;
		s.Format("case when %s < %s then %s else %s end", Str.operator LPCTSTR(), CDateToPeriod(BegLim).operator LPCTSTR(), CDateToPeriod(BegLim).operator LPCTSTR(), Str.operator LPCTSTR());
		return s;
	};
	return Str;
}
...
void CMetaNameParser::BuildRegTurnoversSubQuery(CRegDef* RegDef, const PMetaDataObjArray & Dims, const PMetaDataObjArray & Figs, const CString & Joines, const CString & Conditions, const stTurnoversPlan TurnoversPlan, CString & SubQuery)
{
	...
	if (TurnoversPlan.EnlargeStr.IsEmpty())
	{
		...
		{
			TmpStr.Format("sum(case when %s.debkred = 0 then %s.%%id else 0 end) as %%cdПриход,\r\nsum(case when %s.debkred = 1 then %s.%%id else 0 end) as %%cdРасход", sTurnName.operator LPCTSTR(), sTurnName.operator LPCTSTR(), sTurnName.operator LPCTSTR(), sTurnName.operator LPCTSTR());
			AddMDObjArray(SubQuery, Figs, TmpStr, ",");
		}
	}
	else
	{
		TmpStr.Format("case when %s.debkred = 0 then %s.%%id else 0 end as %%cdПриход,\r\ncase when %s.debkred = 1 then %s.%%id else 0 end as %%cdРасход", sTurnName.operator LPCTSTR(), sTurnName.operator LPCTSTR(), sTurnName.operator LPCTSTR(), sTurnName.operator LPCTSTR());
	...
	TmpStr.Format("where %s.date_time_iddoc > %s and %s.date_time_iddoc < %s\r\n", sDateTimeName.operator LPCTSTR(), GetVTStringParam(TurnoversPlan.BeginBorder).operator LPCTSTR(), sDateTimeName.operator LPCTSTR(), GetVTStringParam(TurnoversPlan.EndBorder).operator LPCTSTR()); SubQuery += TmpStr;
	if (!RegDef->GetDateTimeInActs())
	{
		TmpStr.Format("and %s.rf%d = 0x1\r\n", sDateTimeName.operator LPCTSTR(), RegDef->GetID()); SubQuery += TmpStr;
	};
	...
	if (TurnoversPlan.EnlargeStr.IsEmpty())
	{
		...
		{
			TmpStr.Format("sum(case when %s.debkred = 0 then %s.%%id else 0 end) <> 0 or\r\nsum(case when %s.debkred = 1 then %s.%%id else 0 end) <> 0", sTurnName.operator LPCTSTR(), sTurnName.operator LPCTSTR(), sTurnName.operator LPCTSTR(), sTurnName.operator LPCTSTR());
...
void CMetaNameParser::BuildRegTurnovers(CRegDef* RegDef, CString & SubQuery, CStringList & Params, CString & TmpAlias)
{
	...
	if (RegDef->IsTurnovers() && 
		...
		(RegPeriod != eTenDays && RegPeriod != eWeek && TurnoversPlan.Periodicity != eTenDays && RegPeriod <= TurnoversPlan.Periodicity) ||
		(RegPeriod == eTenDays && TurnoversPlan.Periodicity == eMonth) ||
		...
		))
	{
		...
		else
		{
			...
			else
			{
				...
				StrWhere.Format("%s.date_time_iddoc > %s and %s.date_time_iddoc < %s", sDateTimeName.operator LPCTSTR(), GetVTStringParam(BeginBorder).operator LPCTSTR(), sDateTimeName.operator LPCTSTR(), GetVTStringParam(CDateToCString(BegPer)).operator LPCTSTR());
			};
		};
		...
		else
		{
			...
			else
			{
				...
				TmpStr.Format("%s.date_time_iddoc > %s and %s.date_time_iddoc < %s", sDateTimeName.operator LPCTSTR(), GetVTStringParam(CDateToCString(GetBegOfPeriod(EndDate, RegPeriod))).operator LPCTSTR(), sDateTimeName.operator LPCTSTR(), GetVTStringParam(EndBorder).operator LPCTSTR());
				if (!StrWhere.IsEmpty())
				{
					StrWhere.Insert(0, '(');
					StrWhere += " or ";
	...
	if (fOptimization)
	{
		...
		if (StrWhere.IsEmpty())
		{
			...
			if (sBegPer == sEndPer)
				TmpStr.Format("where %s.period = %s\r\n", sTotName.operator LPCTSTR(), sBegPer.operator LPCTSTR());
			else
				TmpStr.Format("where %s.period between %s and %s\r\n", sTotName.operator LPCTSTR(), sBegPer.operator LPCTSTR(), sEndPer.operator LPCTSTR());
			SubQuery += TmpStr;

			switch (RegPeriod)
			{
			case eQuart:
				TmpStr.Format(" and month(%s.period) in (3, 6, 9, 12)\r\n", sTotName.operator LPCTSTR()); SubQuery += TmpStr;
				break;
			case eYear:
...
void CMetaNameParser::BuildRegRestsTurnovers(CRegDef* RegDef, CString & SubQuery, CStringList & Params, CString & TmpAlias)
{
	...
	else
	{
		...
		typedef std::vector<CRow_allocator::stRow> Turns_vector;
		Turns_vector vecTurns;

		while (fTurnIsNext) {
			TurnRow_allocator_b.SetRowData(Recordset);
			vecTurns.push_back(TurnRow_allocator_b.m_Row);
		...
		*/
		...
		typedef std::vector<CRow_allocator::stRow> Rests_vector;
		Rests_vector vecRests;

		while (fRestIsNext) {
			RestRow_allocator.SetRowData(Recordset);
			vecRests.push_back(RestRow_allocator.m_Row);
		...
		*/
		...
		CString CreateQuery;
		CString sql_type;
		TmpStr.Format("create table %s (\r\n", SubQuery.operator LPCTSTR()); CreateQuery += TmpStr;
		for (int i = 0; i < Dims.GetSize(); i++)
		{
			const CType & t = static_cast<CMetaDataTypedObj *>(Dims[i])->GetType();
			switch(t.GetTypeCode())
			{
			case 0: sql_type = "char(23) collate database_default";
				break;
			case 1: sql_type.Format("numeric(%d,%d)", t.GetPrecision()>0?t.GetLength()-1:t.GetLength(), t.GetPrecision());
				break;
			case 2: sql_type.Format("char(%d) collate database_default", t.GetLength());
				break;
			case 3: sql_type = "datetime";
				break;
			default:
				if (!t.GetTypeID())
					sql_type = "char(13) collate database_default";
				else
					sql_type = "char(9) collate database_default";
			}
			TmpStr.Format("%s %s,\r\n", Dims[i]->GetCode(), sql_type.operator LPCTSTR()); CreateQuery += TmpStr;
		}
		if (TurnoversPlan.Periodicity == e_Document)
			CreateQuery +=  "ПозицияДокумента char(23) collate database_default, ВидДокумента int,\r\n";
		else
		...
		for (int i = 0; i < Figs.GetSize(); i++)
		{
			...
			CurBalance.FactorOfAccuracy = ::pow((double)10, -Prec);
		...
		*--ParamsStr = ')';
		...
		Turns_vector::const_pointer PrevTurnsRow;
			...
			if (ResT)
			{
				if (MethodRT == eActionsBoundaries && TurnsRow != vecTurns.begin())
					if (CurBalancesNotEQ(CurBalances))
						if (TurnRow_allocator_e.CompareRowPeriod(PrevTurnsRow))
				...
				if (ResR)
					for (CvecCurBalances::iterator CurBalance = CurBalances.begin(); CurBalance != CurBalances.end(); CurBalance++)
						CurBalance->Balance = 0;
				else
				{
					SetBalancesFromArray(CurBalances, RestsRow->m_pFigs);
				...
				if (MethodRT == eActionsBoundaries && !ResR)
					if (TurnRow_allocator_b.CompareRowPeriod(&*TurnsRow))
...
void CMetaNameParser::InsertRow(const CRow_allocator::stRow* pRowSource,
								BOOL IsActions,
								...
								CvecCurBalances & CurBalances)
{
	Row_allocator.SetDimsToRecordset(InsRecordset, pRowSource->m_pDims);
	if (IsActions)
	{
		Row_allocator.SetPeroidToRecordset(InsRecordset, pRowSource->m_pDims);

		CvecCurBalances::iterator CurBalance = CurBalances.begin();
		for (int i = 0; i < InsPlan.FigsCount; i++) 
		{
			int nFigField = InsPlan.nFirstFigField + (i<<2);

			double Receipt = Row_allocator.GetFig(pRowSource->m_pFigs, i, 0);
			double Expense = Row_allocator.GetFig(pRowSource->m_pFigs, i, 1);
	...
	{
		Row_allocator.SetPeroidToRecordset(InsRecordset, Row_allocator.m_Row.m_pDims);
...
void CMetaNameParser::BuildLastValue(const CMetaDataTypedObj * MDObj, CString & SubQuery, CStringList & Params)
{
	...
	case UNDEFINE_TYPE_1C:
		...
		break;
	case NUMBER_TYPE_1C:
		TmpStr.Format("cast(%s.value as numeric(%d, %d))\r\n", sAlias.operator LPCTSTR(), MDObj->GetType().GetLength() - !!MDObj->GetType().GetPrecision(), MDObj->GetType().GetPrecision());
		SubQuery += TmpStr;
		break;
	case STRING_TYPE_1C:
		TmpStr.Format("left(%s.value, %d)\r\n", sAlias.operator LPCTSTR(), MDObj->GetType().GetLength());
		SubQuery += TmpStr;
		break;
	case DATE_TYPE_1C:
		{
		...
		break;
	default:
		TmpStr.Format("left(%s.value, %d)\r\n", sAlias.operator LPCTSTR(), 9 + 4 * !MDObj->GetType().GetTypeID());
		SubQuery += TmpStr;
		break;
	};
	SubQuery += "from\r\n_1sconst as ";
	SubQuery += sAlias;
	SubQuery += " (nolock)\r\nwhere\r\n";
	TmpStr.Format("%s.id = %d and\r\n", sAlias.operator LPCTSTR(), MDObj->GetID()); SubQuery += TmpStr;
	TmpStr.Format("%s.objid = %s and\r\n", sAlias.operator LPCTSTR(), ObjID.operator LPCTSTR()); SubQuery += TmpStr;
	SubQuery += '(';
	SubQuery += sAlias;
	SubQuery += ".date <";
	if (Time.IsEmpty())
	...
	if (!Time.IsEmpty())
	{
		TmpStr.Format(" or %s.date = %s and %s.time <", sAlias.operator LPCTSTR(), Date.operator LPCTSTR(), sAlias.operator LPCTSTR()); SubQuery += TmpStr;
		if (DocID.IsEmpty())
			SubQuery += '=';
		SubQuery += ' ';
		SubQuery += Time;
		if (!DocID.IsEmpty())
		{
			TmpStr.Format(" or %s.date = %s and %s.time = %s and %s.docid <= %s", sAlias.operator LPCTSTR(), Date.operator LPCTSTR(), sAlias.operator LPCTSTR(), Time.operator LPCTSTR(), sAlias.operator LPCTSTR(), DocID.operator LPCTSTR()); SubQuery += TmpStr;
		};
	};
	SubQuery += ')';
	TmpStr.Format("\r\norder by %s.date desc, %s.time desc, %s.docid desc, %s.row_id desc\r\n", sAlias.operator LPCTSTR(), sAlias.operator LPCTSTR(), sAlias.operator LPCTSTR(), sAlias.operator LPCTSTR()); SubQuery += TmpStr;
}
...
void CMetaNameParser::BuildBegOfPeriod(CString & SubQuery, const CString & StrPeriodType, CStringList & Params)
{
	...
	CString Source = Params.GetNext(ListPos);
	CString StrSourceType = Params.GetNext(ListPos);
	...
	if (StrSourceType.IsEmpty())
		StrSourceType = "Строка";

	int SourceType;
	if (!StrSourceType.CollateNoCase("Дата") || !StrSourceType.CollateNoCase("Date"))
		SourceType = 1;
	else if (!StrSourceType.CollateNoCase("Строка") || !StrSourceType.CollateNoCase("String"))
		SourceType = 0;
	else
		RaiseException(ExceptStr[eiWrongParam] + DescParameter(2));

	SubQuery = GetPeriodField(Per, Source, SourceType, CDate(0, 0, 0));
}
	...
	while (CurChar = pQT[CurIndex++])
	{
		...
		{// идет типизирующий алиас
			...
			if (!fExtName)
			{
				...
				if(TypingAlias.Replace(buf, TypeNamePrefix))
					TypingAlias.Replace(".", szDotSubst);
...
{
	if (!m_rs.Open("select curdate, curtime, eventidta from _1ssystem (nolock)"))
	{
	...
	CString iddoc = pDBField[2].AsString();
	m_rs.Close();

	TimeTA.SetlTime(time);
	
	long id = CharIDToLong36(iddoc.Left(6),6);
	CObjID objid(id, CDBSign(iddoc.Right(3)));
	TimeTA.SetEventID(objid);
