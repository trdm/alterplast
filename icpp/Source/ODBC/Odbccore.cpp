...
int CODBCRecordset::GetFieldIndex(const char *szFieldName) const
{
  ...
  int ret = -1;
  void *p;
  if (mapName_IndexFields.Lookup(szFieldName, p))
    ret = (int)p;
        ...
        case SQL_C_SLONG:
          {
			...
			case eRefKindAsStr:
				{
					value.SetType(m_rt);
					CMetaDataObj* pObj = m_pRS->m_pDb->m_pMetaDataCont->m_STypeDefs.GetItem(nVal);
					value = pObj ? pObj->GetCode() : "";
				}
				break;
			case eRefKindPresentAsStr:
				{
					value.SetType(m_rt);
					CMetaDataObj* pObj = m_pRS->m_pDb->m_pMetaDataCont->m_STypeDefs.GetItem(nVal);
					value = pObj ? pObj->GetRealPresent() : "";
        ...
        case SQL_C_CHAR:
          {
            ...
            else
            {
				...
				else if (m_AddType == eRefKindAsStr)
				{
					value.SetType(m_rt);
					LONG nVal = strtol(pstring, NULL, 36);
					CMetaDataObj* pObj = m_pRS->m_pDb->m_pMetaDataCont->m_STypeDefs.GetItem(nVal);
				...
				else if (m_AddType == eRefKindPresentAsStr)
				{
					value.SetType(m_rt);
					LONG nVal = strtol(pstring, NULL, 36);
					CMetaDataObj* pObj = m_pRS->m_pDb->m_pMetaDataCont->m_STypeDefs.GetItem(nVal);
    ...
    case AGREGATE_TYPE_1C:
        if (m_nCType == SQL_C_BINARY)
        {
            ...
            else
            {
                ...
                {
                    CValue vBD; //CValue vClassName("BinaryData"), vBD;
                    if (static_cast<CComponentClass*>(pCont)->GetBaseClass("BinaryData", vBD))
	...
	if (nPos >= 0)
	{
        ...
        case UNDEFINE_TYPE_1C:
            if (!strType1C.CompareNoCase("Субконто")|| !strType1C.CompareNoCase("Subconto"))
                addType = eSbKind;
            else if (!strType1C.CompareNoCase("ВидДокумента")|| !strType1C.CompareNoCase("KindDocument"))
			{
                addType = eDocKindAsStr;
				Type.SetTypeCode(STRING_TYPE_1C);
			}
            else if (!strType1C.CompareNoCase("ВидДокументаПредставление")|| !strType1C.CompareNoCase("KindDocumentPresent"))
			{
                addType = eDocKindPresentAsStr;
				Type.SetTypeCode(STRING_TYPE_1C);
			}
            else if (!strType1C.CompareNoCase("ВидСправочника")|| !strType1C.CompareNoCase("KindReference"))
			{
                addType = eRefKindAsStr;
				Type.SetTypeCode(STRING_TYPE_1C);
			}
            else if (!strType1C.CompareNoCase("ВидСправочникаПредставление")|| !strType1C.CompareNoCase("KindReferencePresent"))
			{
                addType = eRefKindPresentAsStr;
				Type.SetTypeCode(STRING_TYPE_1C);
			}
            break;
            /*
