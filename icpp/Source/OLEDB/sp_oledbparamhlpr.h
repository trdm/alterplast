...
private:
	typedef std::vector<COLEDBParam> ParametersContainer_t;

	BOOL FillParamsBuf();
	
	std::vector<BYTE> m_ParamsBuffer;
	int m_nFullSize;
	DBPARAMS m_Params;

	ParametersContainer_t m_ParamList;
public:
	std::vector<DBPARAMBINDINFO> m_ParamBindInfo;
	std::vector<DBBINDING> m_DBBinding;
	std::vector<ULONG> m_ParamOrdinals;
