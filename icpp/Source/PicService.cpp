...
//_____________________________________________________________________________
//
BOOL CPicService::SelectPictureHolder7(eForm eType, const CValue* pValue)
{
	if(CPicService::Picture == eType)
		m_pPicHolder7 = new CPictureHolder7(reinterpret_cast<CPictureContext*>(pValue->GetContext())->m_PictureHolder7);
	else if(CPicService::Gallery == eType)
	{
...
BOOL CPicService::GetRenderedPicture(IPicture** ppPicture)
{
	...
	if(m_IsRendered)
	{
		...
		//ppPicture[0]->AddRef(); // из-за этого идет утечка - 
	}
	else
		bRet = FALSE;
