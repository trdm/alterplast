...
int  CMetaDataWork::CallAsFunc(int iMethNum,CValue & rValue,CValue * *ppValue)
{
  ...
  case funcGetAddPeriod:
    {
      ...
      else
      {
        if(pPar3->GetTypeCode() == STRING_TYPE_1C)
        {
          ...
          {
            CString err;
            err.Format("Не существует регистра с идентификатором: %s", pPar3->GetString().operator LPCTSTR());
  ...
  case funcGetStringInVL:
    {
      ...
      if(pValListCont)
        ...
        CValueListContextData* pVLCD = ((CValueListContextData*)pValListCont->GetInternalData());
			  PValueItemArrayT *pValItAr = reinterpret_cast<PValueItemArrayT*>(pVLCD->GetValueList());
        ...
        if (size > 0)
        {
          type = pValItAr->GetAt(0)->GetTypeCode();
        }
        ...
        for (int i = 0; i < size; ++i)
        {
          ...
          case NUMBER_TYPE_1C:
            {
              res += pVal->GetString(); // TODO возможная ошибка - artbear
              res += ',';
            }
            break;
          case STRING_TYPE_1C:
  ...
  case funcGetBegOfPeriodForReg:
    {
      ...
      if(ppValue[0]->GetTypeCode() == STRING_TYPE_1C)
      {
        ...
        {
          CString err;
          err.Format("Не существует регистра с идентификатором: %s", ppValue[0]->GetString().operator LPCTSTR());
          CBLModule::RaiseExtRuntimeError(err, 0);
        }
      }
  ...
  case methIntToBase:
	...
	case methGetConstID:
    {
		  if(ppValue[0]->GetTypeCode() == STRING_TYPE_1C)
      {
        ...
        {
          CString err;
          err.Format("Не существует константы с идентификатором: %s", ppValue[0]->GetString().operator LPCTSTR());
          CBLModule::RaiseExtRuntimeError(err, 0);
        }
      }
		  else if (ppValue[0]->GetTypeCode() == NUMBER_TYPE_1C)
	...
	case methGetEnumAttributeID:
		{
			...
			CMetaDataObj * pMDO = pMD->GetItem(ppValue[0]->GetString());
			if (!pMDO)
				RuntimeError("Не существует перечисления с идентификатором: %s", ppValue[0]->GetString().operator LPCTSTR());
			...
			for(int __f = 0; __f < __i; __f++)
			{
				...
				if(!pMD_ch)
					RuntimeError("У перечисления \"%s\" нет значений", ppValue[0]->GetString().operator LPCTSTR());
				
				CMetaDataObj * pMDO_ch = pMD_ch->GetItem(ppValue[1]->GetString());
				if (!pMDO_ch)
					RuntimeError("У перечисления \"%s\" не существует значения с идентификатором: %s", ppValue[0]->GetString().operator LPCTSTR(), ppValue[1]->GetString().operator LPCTSTR());
	...
	case methGetRefID:
    {
      if(ppValue[0]->GetTypeCode() == STRING_TYPE_1C)
      {
        ...
        {
          CString err;
          err.Format("Не существует справочника с идентификатором: %s", ppValue[0]->GetString().operator LPCTSTR());
          CBLModule::RaiseExtRuntimeError(err, 0);
        }
      }
		  else if (ppValue[0]->GetTypeCode() == NUMBER_TYPE_1C)
	...
	case methGetRefFieldID:
    {
		  ...
		  if(ppValue[1]->GetTypeCode() == STRING_TYPE_1C)
      {
        ...
        {
          CString err;
          err.Format("Не существует реквизита: %s", ppValue[1]->GetString().operator LPCTSTR());
          CBLModule::RaiseExtRuntimeError(err, 0);
        }
	...
	case methGetDocID:
    {
      if(ppValue[0]->GetTypeCode() == STRING_TYPE_1C)
      {
        ...
        {
          CString err;
          err.Format("Не существует документа с идентификатором: %s", ppValue[0]->GetString().operator LPCTSTR());
          CBLModule::RaiseExtRuntimeError(err, 0);
        }
      }
		  else if (ppValue[0]->GetTypeCode() == NUMBER_TYPE_1C)
	...
	case methGetCJID:
    {
      if(ppValue[0]->GetTypeCode() == STRING_TYPE_1C)
      {
        ...
        {
          CString err;
          err.Format("Не существует журнала расчетов с идентификатором: %s", ppValue[0]->GetString().operator LPCTSTR());
          CBLModule::RaiseExtRuntimeError(err, 0);
        }
      }
		  else if (ppValue[0]->GetTypeCode() == NUMBER_TYPE_1C)
	...
	case methGetRegDimensionID:
    {
		  ...
		  if(ppValue[1]->GetTypeCode() == STRING_TYPE_1C)
      {
        ...
        {
          CString err;
          err.Format("Не существует изерения: %s", ppValue[1]->GetString().operator LPCTSTR());
          CBLModule::RaiseExtRuntimeError(err, 0);
        }
	...
	case methGetRegResourceID:
    {
		  ...
		  if(ppValue[1]->GetTypeCode() == STRING_TYPE_1C)
      {
        ...
        {
          CString err;
          err.Format("Не существует ресурса: %s", ppValue[1]->GetString().operator LPCTSTR());
          CBLModule::RaiseExtRuntimeError(err, 0);
        }
...
int  CMetaDataWork::CallAsProc(int iMethodNum,CValue * * ppValue)
{
  int res = 0;
  switch (iMethodNum)
  {
...
int  CMetaDataWork::GetParamDefValue(int iMethodNum,int iParamNum,CValue * pDefValue)const
{
...
void  CMetaDataWork::GetExactValue(CValue & vParam)
{
...
int  CMetaDataWork::GetPropVal(int nProp, CValue &value)const
{
	if (nProp == 0)
	{
		if (!m_pMetadataContext)
...
CType   CMetaDataWork::GetValueType(void)const
{
	CType tType(100);
	return tType;
}
...
//void  CMetaDataWork::InitObject(CType const & tType)
//{
...
int  CMetaDataWork::SaveToString(CString & csStr)
{
	csStr = "DJK";
	return 1;
}
...
int  CMetaDataWork::SetPropVal(int iPropNum,CValue const & vValue)
{
	return 0;
}
        ...
        if (enTypeStr == VeryLongString)
        {
           switch (TypeCode)
            ...
            case UNDEFINE_TYPE_1C:
              ...
              break;
            case NUMBER_TYPE_1C:
                sprintf(pCurStr, "N%22s", Value.GetString().operator LPCTSTR());
              break;
            case STRING_TYPE_1C:
                sprintf(pCurStr, "S%-22.22s", Value.GetString().operator LPCTSTR());
              break;
            case DATE_TYPE_1C:
              {
