	...
	if (m_pMod)
	{
		...
		if (m_mapOfLinkModules.Lookup(m_pMod, pVector))
		{
			...
			if (pVector)
				...
				pVector->erase(std::remove(pVector->begin(), pVector->end(), this), pVector->end()); 
		}
	}
}
...
typedef CBLModule* (CBLModule::*PF_CBLModuleConstructor2)(class CBLContext *,char const *);
...
CBLModule* CSafeModulePtr::WrapCBLModuleConstructor1(class CBLModule const & module)
{
	...
	CBLModule* pMod = reinterpret_cast<CBLModule*>(this);
...
//LogErr("WrapCBLModule7Constructor1 модуль %d", pMod);
	...
	return (pMod->*wrapper.GetOrigMethod())(module);
...
//LogErr("WrapCBLModule7Constructor2 модуль %d", pMod);
	...
	return (pMod->*wrapper.GetOrigMethod())(pCont, szString);
...
//LogErr("WrapCBLModule7Destructor модуль %d", pMod);
	...
	if (m_mapOfLinkModules.Lookup(pMod, pVector))
	{
		...
		if (pVector)
			std::for_each(pVector->begin(), pVector->end(), ClearModulePtr);
...
{
//return;// TODO временное отключение для 3.0
