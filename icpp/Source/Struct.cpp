...
int  CStruct::CallAsFunc(int iMethNum,CValue & rValue,CValue** ppValue)
{
	switch(iMethNum)
	{
    case funcCount:
    ...
    case funcProperty:
		{
			...
			LPCSTR szKey = strKey.operator LPCTSTR();
...
int  CStruct::CallAsProc(int iMethNum,CValue * * ppValue)
{
	switch(iMethNum)
	{
    case procInsert:
...
int  CStruct::GetParamDefValue(int iMethodNum,int iParamNum,CValue * pDefValue)const
{	
	switch(iMethodNum)
	{
    case procInsert:
...
CType   CStruct::GetValueType(void)const
{
	CType tType(100);
...
int  CStruct::SaveToString(CString & csStr)
{
	csStr = "DJK";
	return 1;
}
...
int  CStruct::GetPropVal(int iPropNum,CValue & rValue)const
{
...
int  CStruct::SetPropVal(int iPropNum,CValue const & vValue)
{
	InnerNode *pInNode = m_Array[iPropNum];
	if (pInNode)
		if (pInNode->bIsPtrVal)   
...
//CBLContext* CStruct::Clone(LPCSTR szClassName) const
//{
//	return new CStruct();
//}
