...
//	CObjID m_ObjID1; // 48h
//	CObjID m_ObjID2; //50h
//	DWORD m_flag58; // 58h - в конструкторе = -1
//	DWORD m_flag5—; // 5—h - в конструкторе = -1
//	DWORD m_buf60[4]; // 60h
//	CString m_string1; // 70h - в конструкторе = "WindowName"
//	CString m_string2; // 74h - в конструкторе = "WindowName"
//	DWORD m_buf78[3]; // 78h
...
BOOL CInformer::HaveContextMethodOrProp(bool bNeedMethods, CValue &RetVal, CValue **ppValue)
{
	...
	if (!pCont)
		if (bNeedMethods)
			RuntimeError("»нформатор::ћетод—уществует - первым параметром нужно передать контекст");
		else
			RuntimeError("»нформатор::—войство—уществует - первым параметром нужно передать контекст");
	...
	if (2 != ValueParam2.GetTypeCode())
		if (bNeedMethods)
			RuntimeError("»нформатор::ћетод—уществует - вторым параметром нужно передать название метода (—трока)");
		else
			RuntimeError("»нформатор::—войство—уществует - вторым параметром нужно передать название метода (—трока)");
...
BOOL CInformer::funcGetIdleTime(CValue &RetVal, CValue **ppValue)
{
	...
	#endif
			RuntimeError("ћетод <»нформатор::ѕолучить¬рем€ѕросто€—истемы> - системна€ ошибка %s", strLastError().operator LPCTSTR());
...
BOOL CInformer::funcGetAddInDllTable(CValue &RetVal, CValue **ppValue)
{
	...
	for(int i = 0; i < sz; i++)
	{
		...
		if ( !FastCompare.Compare(FilePath, "<Ќеизвестна€ компонента>") )
