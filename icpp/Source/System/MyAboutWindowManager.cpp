...
void aux_AddLines(CString& text, LPCSTR szCaption, LPCSTR szLines)
{
	CString val(szLines);
	if (!val.IsEmpty())
	{
		...
		val.Replace("\r\n", "\\par ");
		CString tmp;
		tmp.Format("\\ul1%s \\ul0\\par %s\\par ", szCaption, (LPCSTR)val);
		text += tmp;
	}
}
		...
		if(!m_bLogoShowed)
		{
			...
			if(hWnd)
			{
				...
				text = "{\\rtf1\\ansi\\deff0\\deftab720"
					...
					"\\b1Рыбальченко Степан aka steban\\b0\\par класс ActiveX\\par"
				...
				if (pHierarchy)
				{
					tmp.Format("\\fs20\\b1\\ul1Информация о классах:\\b0\\ul0\\par\\fs10\\par"
						"Всего зарегистрировано классов: %i\\par", MetaDataOfClasses.GetCountClasses());
					text += tmp;

					tmp.Format("\\Всего объявлено методов: %i\\par\\par", MetaDataOfClasses.GetCountMethods());
					text += tmp;

 					aux_AddLines(text, "Определенные директивы препроцессора:", pHierarchy->GetDefSymbolOfPreproc());
 					aux_AddLines(text, "Вложенные файлы:", pHierarchy->GetLoadIncludeFiles());
 					aux_AddLines(text, "Имена зарегистрированных классов:", MetaDataOfClasses.GetAllClassesNames());
 					aux_AddLines(text, "Имена псевдонимов и их пути:", MetaDataOfClasses.GetAllAliasAndPath());
				}

				CString val(pMainAddIn->GetPropertyInfo());
				val.Replace("\r\n", "\\par ");
				tmp.Format("\\fs20\\b1\\ul1Информация о настройках:\\b0\\ul0\\par\\fs10\\par %s", (LPCSTR)val);
				text += tmp;
				text += " }";
