...
int  CAddInContext_MyProxy::CallAsFunc(int iMethNum,CValue & rValue,CValue** ppValue)
{
	if (!m_BaseCont)
...
int  CAddInContext_MyProxy::CallAsProc(int iMethNum,CValue * * ppValue)
{
	if (!m_BaseCont)
...
int  CAddInContext_MyProxy::GetParamDefValue(int iMethodNum,int iParamNum,CValue * pDefValue)const
{	
	if (!m_BaseCont)
...
void  CAddInContext_MyProxy::GetExactValue(CValue & vParam)
{
	if (!m_BaseCont)
...
CType   CAddInContext_MyProxy::GetValueType(void)const
{
// 	CType tType(100);
// 	
// 	return tType;
...
void  CAddInContext_MyProxy::InitObject(CType const & tType)
{
...
int  CAddInContext_MyProxy::SaveToString(CString & csStr)
{
	if (!m_BaseCont)
...
int  CAddInContext_MyProxy::GetPropVal(int iPropNum,CValue & rValue)const
{
	if (!m_BaseCont)
...
int  CAddInContext_MyProxy::SetPropVal(int iPropNum,CValue const & vValue)
{
	if (!m_BaseCont)
