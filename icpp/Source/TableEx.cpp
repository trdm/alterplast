...
int CTableWrapEx::procAddAllMDInnerFormsName(CString &str,long l1,long l2,enum PageType pt)
{
...
CTableWrapEx::CSetTableReturnValue CTableWrapEx::SetTableForMDInnerForms(CString& strName, const CString& strPath)
{
	...
	if(!DocNamesMap.Lookup(strDoc, pDocNames))
	...
	CString strFull = "";
	strFull.Format("%s\\%s\\%s_Number%d\\WorkBook", Prop8.operator LPCTSTR(), pDocNames->GetFolderName(), pDocNames->GetFolderName(), pDocNames->GetID());
...
CTableWrapEx::CSetTableReturnValue CTableWrapEx::SetTableForNotOpenedErtFile(CString& strName, const CString& strPath)
{
	...
	std::vector<char> buf(StreamLen); //char* buf = (char*)malloc(StreamLen);
	...
	{
		CGenericItem* pGenericItem = list1.GetItemAt(j);
		const int w = pGenericItem->WhatIsIt();
		if(w != 1)
			continue;

		CItemList* pItemList = static_cast<CItemList*>(pGenericItem);
		CItem* pItem1 = static_cast<CItem*>(pItemList->GetItemAt(1));
		CItem* pItem2 = static_cast<CItem*>(pItemList->GetItemAt(2));
		
		CString& strItem2String = pItem2->m_csFunctor;
		if (!strName.CompareNoCase(strItem2String) ||
			(!strItem2String.CompareNoCase("Moxel WorkPlace") &&
			 !strName.CompareNoCase("Таблица")))
		{
			strStreamName = pItem1->m_csFunctor;
...
int CTableWrapEx::CallAsProc(int iMethNum,CValue **ppValue)
{
...
int CTableWrapEx::CallAsFunc(int iMethNum,CValue & rValue,CValue **ppValue)
{
	//if(strMeth.CompareNoCase("ИсходнаяТаблица") == 0)
	if(iMethNum == iMethNum_SourceTable)
	{
...
int CTableWrapEx::CallOriginalMethod_SourceTable(CBLContext* pCont, CValue **ppValue)
{
...
int CTableWrapEx::HasRetVal(int iMethNum)const
{
	...
	return (((CTableWrapEx *)this)->*pHasRetVal)(iMethNum);
};
...
int CTableWrapEx::GetNParams(int iMethNum)const
{
	...
	return (((CTableWrapEx *)this)->*pGetNParams)(iMethNum);
};

int CTableWrapEx::GetParamDefValue(int iMethNum,int iParamNum,CValue * pDefValue)const
{
	//if(strMeth.CompareNoCase("ИсходнаяТаблица") == 0)
	if(iMethNum == iMethNum_SourceTable)
	{
	...
	return (((CTableWrapEx *)this)->*pGetParamDefValue)(iMethNum, iParamNum, pDefValue);
};
...
void CTableWrapEx::ChangeTableOutputContextVirtualTable()
{
	...
	boost::scoped_ptr<CTableWrapEx> pTableWrapEx(new CTableWrapEx);
...
{
	boost::scoped_ptr<CTableWrapExDbl> pTableWrapExDbl(new CTableWrapExDbl);
...
int CTableWrapExDbl::CallAsProc(int iMethNum,CValue **ppValue)
{
