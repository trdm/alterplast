...
BEGIN_BL_METH_MAP(CVTExtended)
	...
	BL_METH_PROC_DEF_PARAM("FillColumnByAlgorythm",  "ЗаполнитьКолонкуПоАлгоритму",  4, &procFillColumnByAlgorithm, &defsFillColumnByAlgorithm)

	BL_METH_PROC_DEF_PARAM("UnloadRow",	"ВыгрузитьСтроку",    4, &procUnloadRowTo, &defsUnloadRowTo)
...
#define SERIALIZE_TO_STRING_PREFIX "VTExt"
...
CVTExtended::CVTExtended()
{
	if( FixedProps.GetCount() == 0 )
	{
		...
		CSerializeToString::Add(SERIALIZE_TO_STRING_PREFIX, GetRuntimeClass()->m_lpszClassName, GetTypeString());
	}
...
int CVTExtended::NewColumn(CString const& ColumnName)
{
	...
	{
		RuntimeError("Колонка %s уже есть!", ColumnName.operator LPCTSTR());
		return -1;
	}
...
BOOL CVTExtended::procRenameColumn(CValue** params)
{
	...
	if( ColumnNumber(NewName, true) >= 0 )
		RuntimeError("Колонка '%s' уже есть в таблице", NewName.operator LPCTSTR());
				...
				if (!sIndexExpr.IsEmpty())
				{
					...
					if( nFields )
					{
						...
						for( int i = 0; i < nFields; i++)
						{
							...
							if( nSrcCol < 0 )
								RuntimeError("Неверное имя колонки таблицы-источника \"%s\"", sSrcField.operator LPCTSTR());
							...
							{
								RuntimeError("Неверное имя колонки таблицы-приёмника \"%s\"", sDestField.operator LPCTSTR());
							}
				...
				if (!sIndexExpr.IsEmpty())
				{
					...
					if(nFields)
					{
						...
						for(int i = 0; i < nFields; i++)
						{
							...
							if (nSrcCol < 0)
								RuntimeError("Колонка '%s' не найдена!", sField.operator LPCTSTR());
...
int* CVTExtended::ColumnNumbersMap(CVTExtended* pDest, CValueItemList* ppValueList)
{
...
void CVTExtended::CopyRowFromVL(CVTExtRow *pDest, CValueItemList* ppValueList, int *pColNumbersMap)
{
...
BOOL CVTExtended::procFillRow(CValue** params)
{
	...
	CValueItemList* ppValueList = CValue2VL(Value);
	//param 2
...
void CVTExtended::PartiallyReindex(std::deque<bool>& bChangedRows, int nChangedColumn)
{
	...
	for( int i = GetRowsCount() - 1; i >= 0; i-- )
	{
		if( bChangedRows[i] )
		{
			...
			{
				if( Indexes[nIndex]->ColumnIndexed(nChangedColumn) )
				{
...
BOOL CVTExtended::procFillColumn(CValue** params)
{
	...
	deque<bool> bChangedRows;
	
	if( bNeedReindex )
	{
		bChangedRows.resize(GetRowsCount(), false);
	}
	...
	if( bNeedReindex )
	{
		PartiallyReindex(bChangedRows, nDestCol);
	}

	return TRUE;
}

BOOL CVTExtended::defsFillColumnByAlgorithm(int nParam, CValue* param) const
{
	switch( nParam )
	{
	case 0:
...
// bool IsExecuteModule(const CValue& Value)
// {
//     CBLContext* pCont = Value.GetContext();
// 	if( pCont && strcmp(pCont->GetRuntimeClass()->m_lpszClassName, "CExecuteModule") == 0 )
// 	{
// 		return true;
// 	}
// 	return false;
...
{
public:
	virtual void AssignNext(CValue& val) = 0;
};

template<typename VALTYPE, typename INCRTYPE>
class CValSummatorT : public CValSummator
{
	VALTYPE m_val;
	INCRTYPE m_incr;
public:
	CValSummatorT(VALTYPE const& val, INCRTYPE const& incr)
		: m_val(val), m_incr(incr)
	{
	}

	void AssignNext(CValue& val)
	{
		val = m_val;
		m_val += m_incr;
	}
...
template<>
void CValSummatorT<CNumeric, CNumeric>::AssignNext(CValue &val)
{
	val = m_val;
	m_val = m_val + m_incr;
}
...
template<>
void CValSummatorT<CDate, int>::AssignNext(CValue &val)
{
	val = m_val;
	m_val.m_DateNum += m_incr;
}

CValSummator* CreateValSummator(CValue const& val, CValue const& incr)
{
	int nValType = val.GetTypeCode();
	int nIncrType = incr.GetTypeCode();

	if (nValType == NUMBER_TYPE_1C && nIncrType == NUMBER_TYPE_1C)
	{
		CNumeric const& NumVal = val.GetNumeric();
		CNumeric const& NumIncr = incr.GetNumeric();
		long nVal = NumVal;
		long nIncr = NumIncr;
		if (!NumVal.CompareLong(nVal) && !NumIncr.CompareLong(nIncr))
			return new CValSummatorT<long, long>(nVal, nIncr);
		else
			return new CValSummatorT<CNumeric, CNumeric>(NumVal, NumIncr);
	}
	else if (nValType == STRING_TYPE_1C && nIncrType == STRING_TYPE_1C)
		return new CValSummatorT<CString, CString>(val.GetString(), incr.GetString());
	else if (nValType == DATE_TYPE_1C && nIncrType == NUMBER_TYPE_1C)
		return new CValSummatorT<CDate, int>(val.GetDate(), incr.GetNumeric());

	return NULL;
}

BOOL CVTExtended::procFillColumnByAlgorithm(CValue** params)
{
	CVTExtIndex *pIndex = GetIndex(params[0]);
	int nDestCol = ColumnNumber(params[1]);
	CValue* pFillValue = params[2]; //CBLContext or number/date
	...
	CBLContext* pFunctor = pFillValue->GetContext();
	if( pFunctor )
	{
		int nMethNext = pFunctor->FindMethod("NextValue");
		if (nMethNext == -1)
			RuntimeError("Неверный алгоритм (не определена функция NextValue()!");

		if (!pFunctor->HasRetVal(nMethNext))
			RuntimeError("Алгоритм должен быть объявлен как функция!");

// 		if (pFunctor->GetNParams(nMethNext))
// 			RuntimeError("Алгоритм не должен принимать аргументов!");

		// в тестовых целях разрешаем 1 параметр для поддержки CLuaRef
		// в этом случае в этот параметр передаем сам контекст
		CValue** ppValues  = NULL;
		CallIsOleContext(pFunctor, 1);
		int nParams = pFunctor->GetNParams(nMethNext);
		if (nParams > 1)
			RuntimeError("Алгоритм не должен принимать аргументов!");
		else
			ppValues = &pFillValue;
		...
		{
			if (nParams)
				CallIsOleContext(pFunctor, nParams);
			pFunctor->CallAsFunc(nMethNext, Value, ppValues);
			pIterator->CurrentRow()->SetValue(nDestCol, Value);
			if( bNeedReindex )
				bChangedRows[pIterator->CurrentRowIndex()] = true;
		}
	}
	else
	{
		CValue const& IncrValue = *params[3];
		std::auto_ptr<CValSummator> pValSummator(CreateValSummator(*pFillValue, IncrValue));
		if (!pValSummator.get())
			RuntimeError("Операция суммирования для заданных параметров не определена!");

		pIterator->First();
		while( pIterator->Next() )
		{
			pValSummator->AssignNext(pIterator->CurrentRow()->GetValue(nDestCol));
			if( bNeedReindex )
				bChangedRows[pIterator->CurrentRowIndex()] = true;
		}
	}
...
void CVTExtended::UnloadToVT(CValueTable* pVT, CVTExtIndex* pIndex, CValue* pColumns, bool bUnique)
{
	...
	CDerivedVT* pDVT = static_cast<CDerivedVT*>(pVT);
	BOOL bPrevEnableRefresh = pDVT->m_bEnableRefresh;
	...
	pDVT->m_bEnableRefresh = bPrevEnableRefresh;
	pDVT->DeleteRow(pVT->NewRow(-1));
}
...
void CVTExtended::UnloadToDoc(CDocContext* pDoc, CVTExtIndex* pIndex, bool bUnique)
{
	...
	CTableOfDocResultLoader Loader;
	Loader.setContext(pDoc);

	CValue dummyValue;
	UnloadToResultLoader(&Loader, pIndex, dummyValue, bUnique);
}
...
BOOL CVTExtended::procUnloadTo(CValue** params)
{
	...
	shared_ptr<IResultLoader> pLoader (CValue2ResultLoader(Dest));
	if( pLoader ){
		// предварительная очистка
		CValueTable* pVT = CValue2VT(Dest);
		if (pVT)
			pVT->Clear(TRUE);
		else{
			CValueItemList* pVL = CValue2VL(Dest);
			if (pVL)
				pVL->RemoveAll();
		}

		CVTExtIndex *pIndex = GetIndex(params[1]);
		UnloadToResultLoader(pLoader.get(), pIndex, *params[2], bUnique);
		return TRUE;
	}
	//if( pVT )
	//{
	...
	//{
	//	CVTExtIndex *pIndex = GetIndex(params[1]);
	//	UnloadToDoc(pDoc, pIndex, bUnique); // TODO для табличной части колонок изначально список колонок нельзя задать !!
	//}
	else
	{
		RuntimeError("Требуется переменная с типом ИндексированнаяТаблица, ТаблицаЗначений, СписокЗначений, Документ или любой объект, поддерживающий универсальный интерфейс загрузки!");
	}
	...
	if (bool bAddSrcRows = CValue2Bool(*params[3]))
	{
		...
		for( int nRow = 0; nRow < nSrcRows; nRow++ )
		{
			...
			if( pIndexDest->Search(DestIdxRec) )
			{
				...
				Indexes.InsertRow(NewRow, Rows.Add(NewRow));
			}
		}
...
void CVTExtended::SaveToStorage_CSV(CSerialStorage& Storage, CVTExtIndex *pIndex, bool InnerFormat, bool bWithColNames)
{
	...
	while( pIterator->Next() )
	{
		...
		for( nCol = 0; nCol < nCols; nCol++ )
		{
			...
			Storage.AddString("%s%s%s%s", semicolon, quote, str.operator LPCTSTR(), quote);
...
BOOL CVTExtended::procSaveToFile(CValue** params)
{
	...
	if( f == NULL )
		RuntimeError("Невозможно открыть файл %s! %s", FileName.operator LPCTSTR(), strerror(errno));
		...
		case read_row:
			...
			else 
			{
				...
				else
				{
					...
					CValue &val = pRow->GetValue(nCol);
					if( bInnerFormat )
						LoadValueFromString(&val, (LPCSTR)(str));
					else
					{
						char t;
						sscanf(str, "%c:", &t);
...
bool parse_string(char*& ptr, CString& Token)
{
	...
	bool bInQotes = *ptr == '"'; //very first symbol is '"' - scan the string untill next '"' and 
...
BOOL CVTExtended::LoadFromString(LPCSTR szSource)
{
	CSerialStorage Storage(szSource);
...
int CVTExtended::ColumnNumber(LPCSTR ColName, bool bQuiet) const
{
	int nCol;
...
int CVTExtended::ColumnNumber(CValue* ColNum, bool bQuiet) const
{
	int col_num;
...
int CVTExtended::GetPropVal(int iPropNum, CValue & lValue) const
{
	switch( iPropNum )
	{
...
int CVTExtended::SetPropVal(int iPropNum, CValue const & vValue)
{
	switch( iPropNum )
	{
	case VTExt_FixedProp_CurrentRowRus:
...
int CVTExtended::SaveToString(CString &str)
{
	...
	CString csQuotedPrefix = CSerializeToString::QuotePrefix(SERIALIZE_TO_STRING_PREFIX);
...
CDocContext* CValue2Doc(CValue const& Value)
{
	...
	CValue vBaseClass;
	if( static_cast<IUserClassContext*>(pContext)->GetBaseClass("Документ", vBaseClass) )
	{
	...
	else
	{
		...
		if( !CallAsFunc(pContext, "Вид", vKind, NULL, false) ) return NULL;

		CString sType = "Документ.";
		sType += vKind.GetString();
		//CValue vType(sType);
		//if( static_cast<CComponentClass*>(pContext)->GetBaseClass(vType, vBaseClass) )
		if( static_cast<IUserClassContext*>(pContext)->GetBaseClass(sType, vBaseClass) )
		{
			return (CDocContext*)(vBaseClass.GetContext());
		}
	}
...
bool CallAsFunc(CBLContext* pContext, char const* sFuncName, CValue& RetVal, CValue** ppParams, bool bEnableException)
{
...
bool CallAsProc(CBLContext* pContext, char const* sProcName, CValue** ppParams, bool bEnableException)
{
...
bool GetPropVal(CBLContext* pContext, char const* sPropName, CValue& RetVal, bool bEnableException)
{
...
DWORD CVTExtended::init(DWORD colCountInResult, sqlite3* pBase, CValue* pClear)
{
	if(pClear->IsEmpty() || 0 != pClear->GetNumeric())
	{
		procCleanup(NULL);
		assert(0 == GetColumnsCount()); // в какой-то момент в icpp был глюк с количеством :(
	...
	DWORD colInTables = GetColumnsCount();

	m_colCount = min(colInTables, colCountInResult);
	return m_colCount;
}

// Добавление колонки в результат запроса
void CVTExtended::setColumn(DWORD colIdx, const CString& colName, const CType& type)
{
	if(GetColumnsCount() <= colIdx)
		NewColumn(colName);
}
...
// добавленных колонок
void CVTExtended::addValues(CValue** ppValues)
{
	CVTExtRow* pRow = GetRow(NewRow());
	for( int i = 0; i < m_colCount; i++)
	{
		pRow->SetValue(i, *ppValues[i]);
	}
}

// Присвоение возвращаемого значения
void CVTExtended::assignRetValue(CValue& retVal)
{
	retVal.AssignContext(this);
}
// end of ISQLiteResultLoader

BOOL CVTExtended::defsUnloadRowTo( int nParam, CValue* param ) const
{
	switch( nParam )
	{
	case 1:
		param->SetTypeCode(UNSPECIFIED_TYPE_1C);
		return TRUE;
		break;
	case 2:
		param->Reset();
...
BOOL CVTExtended::procUnloadRowTo( CValue** params )
{
	UnloadRowTo( *params[0], *params[1], *params[2], *params[3]);
	return TRUE;
}

void CVTExtended::GetColumnsListFromValue(const CValue& vColumns, bool bNeedColumnNames, std::pair<std::vector<int>, CStringArrayEx>* pPair) const
{
	// TODO устранить дублирование с UnloadToVT по получению списка колонок и возможно с другими методами выгрузки
	switch (vColumns.GetTypeCode())
	{
	case STRING_TYPE_1C:
		{
			CString sIndexExpr = vColumns.GetString();
			if (!sIndexExpr.IsEmpty())
			{
				CStringArrayEx strArray;
				strArray.FillSeparateString(sIndexExpr, ",");

				int nColumns = strArray.GetSize();
				if(nColumns)
				{
					pPair->first.resize(nColumns);
					for(int i = 0; i < nColumns; i++)
					{
						CString& sField = strArray[i];
						pPair->first[i] = ColumnNumber(sField, false); // TODO если указаны только неверные колонки, то возвратится 0 колонок, и будут выгружены все колонки - неверно !!
						if (bNeedColumnNames)
							pPair->second.Add(ColumnName(pPair->first[i])); // для точного соответствия регистру наименования в ИТЗ,а не как пользователь написал
					}
				}
			}
		}
	...
	case NUMBER_TYPE_1C:
		{
			int nSrcCol = vColumns.GetNumeric().operator long() - 1;
			LPCSTR sField = ColumnName(nSrcCol);
			if (!sField)
				RuntimeError("Неверный номер колонки: %i", nSrcCol + 1);

			pPair->first.push_back(nSrcCol);
			if (bNeedColumnNames)
				pPair->second.Add(sField);
		}
		break;
	default:
		;
	}
	assert(!bNeedColumnNames || bNeedColumnNames && (pPair->first.size() == pPair->second.GetSize()));
}

void CVTExtended::GetFullColumnsList(bool bNeedColumnNames, std::pair<std::vector<int>, CStringArrayEx>* pPair) const
{
	int nColumns = GetColumnsCount();
	pPair->first.resize(nColumns);
	for( int i = 0; i < nColumns; i++ )
...
class IResultLoaderGuard{
	shared_ptr<IResultLoader> m_pLoader;
public:
	IResultLoaderGuard(const shared_ptr<IResultLoader>& pNewLoader) : m_pLoader(pNewLoader) {
		assert(NULL != m_pLoader.get());
	}
	~IResultLoaderGuard() { m_pLoader->ldrDone(); }
	const shared_ptr<IResultLoader>& Get() const { return m_pLoader; }
};

void CVTExtended::UnloadRowTo( CValue const& vDest, CValue const& vRow, CValue const& vColumns, CValue const& vUnloadNames)
{
	const IResultLoaderGuard guard(CValue2ResultLoader(vDest));
	const shared_ptr<IResultLoader>& pLoader(guard.Get()); //shared_ptr<IResultLoader> pLoader (CValue2ResultLoader(vDest));

	if (!pLoader)
		RuntimeError("Неверный тип приемника!");

	size_t nUnloadNamesAtPos = vUnloadNames.GetNumeric(); // 0 = off, 1 = before data, 2 = after data
	if (nUnloadNamesAtPos > 2)
		RuntimeError("Неверное положение наименований колонок!");

	int nRow;
	switch(vRow.GetTypeCode())
	{
	case NUMBER_TYPE_1C:
		nRow = (int)vRow.GetNumeric() - 1;
		break;
	case UNSPECIFIED_TYPE_1C:
		nRow = DefaultIndex->GetCurrentRow();
		break;
	default:
		RuntimeError("Неверный номер строки!");
	}
	// TODO желательно бы добавить по паре inline-методов типа InnerIndex2ExternalIndex и  ExternalIndex2InnerIndex
	// для простого перехода между индексами от 0 (внутр.) и от 1 (наруж. 1С)

	BOOL bUnloadValues = nRow >= 0;
	bool bNeedColumnNames = !nUnloadNamesAtPos ? false : true;

	if (!bNeedColumnNames && !bUnloadValues)
		RuntimeError("Не заданы объекты для выгрузки!");

	CVTExtRow* pCurrentRow = NULL;
	if (bUnloadValues)
	{
		CheckBounds_Row(nRow);
		pCurrentRow = GetRow(nRow);
	}

	// TODO устранить дублирование с UnloadToVT по получению списка колонок и возможно с другими методами выгрузки
	std::pair<std::vector<int>, CStringArrayEx> mypair;
	GetColumnsListFromValue(vColumns, bNeedColumnNames, &mypair);

	if (mypair.first.empty())
	{
		GetFullColumnsList(bNeedColumnNames, &mypair);
	}

	DWORD nLoaderRows = mypair.first.size();
	if (!nLoaderRows)
		RuntimeError("Отсутствуют колонки!");

	DWORD nLoaderColumns = (bNeedColumnNames ? 1 : 0) + (bUnloadValues ? 1 : 0);
	DWORD nLoaderRows_temp = nLoaderRows;
	if (FAILED(pLoader->ldrInit(nLoaderColumns, nLoaderRows_temp)))
		RuntimeError(pLoader->GetLastError());
	...
	if (nLoaderColumns == 1)
	{
		if (bUnloadValues)
		{
			if (nUnloadNamesAtPos == 1)
				bUnloadValues = 0;
			else
				nUnloadNamesAtPos = 0;
		}
		else
			nUnloadNamesAtPos = min(nUnloadNamesAtPos, 1);
	};

	bNeedColumnNames = !nUnloadNamesAtPos ? false : true;

	for (int i = 0; i < nLoaderColumns; i++)
		if (FAILED(pLoader->ldrAddColumn(NULL, NULL)))
			RuntimeError(pLoader->GetLastError());

	CValue vColumnName;
	CValue* pVal[2];
	ptrdiff_t nValOffset = 0;
	if (nUnloadNamesAtPos == 1)
		*pVal = &vColumnName, ++nValOffset;
	else if (nUnloadNamesAtPos == 2)
		*(pVal + 1) = &vColumnName;

	for (int i = 0; i < nLoaderRows; i++)
	{
		int colIndex = mypair.first[i];
		if (bNeedColumnNames)
			vColumnName = mypair.second[i];

		if (bUnloadValues)
		{
			CValue& val = pCurrentRow->GetValue(colIndex);
			pVal[nValOffset] = &val;
		}

		HRESULT hr = pLoader->ldrAddRow(&pVal[0]);
		if (FAILED(hr))
			RuntimeError(pLoader->GetLastError());
		else if (hr != NO_ERROR)
			break;
...
void CVTExtended::UnloadToResultLoader(IResultLoader* pNewLoader, CVTExtIndex* pIndex, const CValue& vColumns, bool bUnique)
{
	// TODO устранить дублирование с UnloadToVT по получению списка колонок и возможно с другими методами выгрузки
	std::pair<std::vector<int>, CStringArrayEx> mypair;
	GetColumnsListFromValue(vColumns, true, &mypair);

	if (mypair.first.empty())
	{
		GetFullColumnsList(true, &mypair);
	}

	DWORD nLoaderColumns = mypair.first.size();
	if (!nLoaderColumns)
		RuntimeError("Отсутствуют колонки!");

	const IResultLoaderGuard guard(shared_ptr<IResultLoader>( pNewLoader, null_deleter<IResultLoader>() ));
	const shared_ptr<IResultLoader>& pLoader(guard.Get()); //shared_ptr<IResultLoader> pLoader (CValue2ResultLoader(Dest));
	...
	DWORD nLoaderRows = 0;
	if( pIndex->IsEmpty() )
		nLoaderRows = GetRowsCount();
	else
		nLoaderRows = pIndex->GetRowsCount(bUnique);

	//if (!nLoaderRows)
	//	RuntimeError("Отсутствуют строки!");

	if (FAILED(pLoader->ldrInit(nLoaderColumns, nLoaderRows)))
		RuntimeError(pLoader->GetLastError());

	for (int i = 0; i < nLoaderColumns; i++)
		if (FAILED(pLoader->ldrAddColumn(mypair.second[i], NULL)))
			RuntimeError(pLoader->GetLastError());

	std::vector<CValue*> ppValues (nLoaderColumns);

	boost::scoped_ptr<CVTExtIterator> pIterator (pIndex->CreateIterator(bUnique));
	pIterator->First();
	while( pIterator->Next() )
	{
		CVTExtRow *pRow = pIterator->CurrentRow();
		for( int nCol = 0; nCol < nLoaderColumns; nCol++ )
			ppValues[nCol] = &pRow->GetValue(mypair.first[nCol]);

		if (FAILED(pLoader->ldrAddRow(&ppValues[0])))
			RuntimeError(pLoader->GetLastError());
	}
}
