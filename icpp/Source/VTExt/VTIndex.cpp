...
*/
		comp_res = CVCompare::CValue_compare(left, right, IdxRec);
		if (comp_res)
		{
...
bool CVTExtIndexFields::Build(CString const& IndexExpr, CVTExtended* pVT)
{
	...
	for( int i = 0; i < nFields; i++ )
	{
		...
		operator[](i).ParseMod(ColName);
		TVTIndexDescrRecord::TrimMod(ColName);
		
		if( pVT )
		{
			int col_num = pVT->ColumnNumber(ColName, false);
			if( col_num < 0 )
				RuntimeError("Колонка '%s' не существует!", ColName.operator LPCTSTR());
	...
	while( node )
	{
		if( node->ArrayEqualIDs && !node->ArrayEqualIDs->empty() )
		{
			...
			if( node->ID == row_num )
			{
				...
				//Возьмём последнюю строку из списка дублей.
				node->ID = Vector.back();
...
void CVTExtIndex::RemoveRow(int nRow)
{
	...
	if( pIterator )
	{
		//TODO: Сеня, пора уже что-то решать...
...
void FillSearchKey(CValueItemList* ppValues, CVTExtIndexFields *pIndexFields, CVTExtRow& KeyRow, CVTIndexRecord& IdxRec)
{
...
{
	CValueItemList* ppVLArray = CValue2VL(What);
...
CVTExtIndexTreeNode* CVTExtIndex::Find(CValueItemList* pKeyValues)
{
	if( !IsIndexed() ) RuntimeError("Таблица не проиндексирована!");
...
void CVTExtIndex::SetFilter(CValue const& SubSet, int nCols)
{
	...
	CValueItemList* ppVLArray = CValue2VL(SubSet);
	if( ppVLArray == NULL )
		RuntimeError("В качестве ключа может использоваться только список значений!");
...
int CVTExtIndex::SaveIterator()
{
	...
	if (pIterator)
	{
		stIteratorState* pState = new stIteratorState(pFilter, pIterator, Version);
...
int CVTExtIndexCollection::AddIndex(CString const& IndexName, CVTExtIndexFields const& IndexFields, bool bUnique, CVTExtended* pVT)
{
	...
	if( GetItem(IndexName) != NULL )
		RuntimeError("Индекс %s уже есть!", IndexName.operator LPCTSTR());
