...
int  CWorkAsRegister::CallAsFunc(int iMethNum,CValue & rValue,CValue * *ppValue)
{
	switch(iMethNum)
	{
	case methCreateKey: 
	case methOpenKey:
		{
			...
			if (str == "HKEY_CLASSES_ROOT") {
				...
				char buf[100];
				sprintf(buf, "Ќеверный ключ: %s", str.operator LPCTSTR());
				pBkEndUI->DoMessageLine(buf,mmExclamation);
				rValue = CNumeric(0);
				return 1;
			}
			...
			if (str == "KEY_CREATE_LINK")
				...
				char buf[100];
				sprintf(buf, "Ќеверно указан режим: %s", str.operator LPCTSTR());
				pBkEndUI->DoMessageLine(buf,mmExclamation);
				rValue = CNumeric(0);
				return 0;
			}
...
int  CWorkAsRegister::CallAsProc(int iMethNum, CValue * * ppValue)
{
	CValue* pValue = NULL;
...
int  CWorkAsRegister::GetParamDefValue(int iMethodNum,int iParamNum,CValue * pDefValue)const
{
	if (iMethodNum >= lastMethod)
	{
		return 1;
...
void  CWorkAsRegister::GetExactValue(CValue & vParam)
{
...
CType   CWorkAsRegister::GetValueType(void)const
{
	CType tType(100);
...
//void  CWorkAsRegister::InitObject(CType const & tType)
//{
...
int  CWorkAsRegister::SaveToString(CString & csStr)
{
...
int  CWorkAsRegister::GetPropVal(int iPropNum,CValue & rValue)const
{
	return pBLModule->GetStaticVarValue(iPropNum,rValue,0);
}

int  CWorkAsRegister::SetPropVal(int iPropNum,CValue const & vValue)
{
