...
void CMyContextBase::InitAllContextClasses()
{
	...
	for(_S_MyContextBaseInfo** ppInfo=&pBeginOfInitData+1;ppInfo<&pEndOfInitData;ppInfo++)
	{
		if(pInfo=*ppInfo)
		{
			...
			CRuntimeClass* pRuntimeClass = const_cast<CRuntimeClass*>(pInfo->pRTC);

			if (pInfo->RegToCreate)
			{
				CBLContext::RegisterContextClass(pRuntimeClass,pInfo->EngTypeString,CType(100));
				CBLContext::RegisterContextClass(pRuntimeClass,pInfo->RusTypeString,CType(100));
			}

            if (pInfo->nFactoryCtorParamCount >= 0)
			{
                CObjectFactory::RegisterClass(pInfo->EngTypeString, pRuntimeClass, pInfo->nFactoryCtorParamCount);
                CObjectFactory::RegisterClass(pInfo->RusTypeString, pRuntimeClass, pInfo->nFactoryCtorParamCount);
			}
	...
	for(_S_MyContextBaseInfo** ppInfo=&pBeginOfInitData+1;ppInfo<&pEndOfInitData;ppInfo++)
	{
		if(pInfo=*ppInfo)
		{
			if(pInfo->pDoneFunc)
				pInfo->pDoneFunc();
			if(pInfo->RegToCreate)
				CBLContext::UnRegisterContextClass(const_cast<CRuntimeClass*>(pInfo->pRTC));
			if (pInfo->nFactoryCtorParamCount >= 0)
			{
				CObjectFactory::UnRegisterClass(pInfo->EngTypeString);
				CObjectFactory::UnRegisterClass(pInfo->RusTypeString);
			}
