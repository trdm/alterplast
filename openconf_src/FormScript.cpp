...
BOOL CFormScript::OnNewDocument()
{
	...
	if(!pItem->CreateNewItem(MsFormClsid))
		return FALSE;
	m_pUserForm=pItem->m_lpObject;
...
BOOL CFormScript::OnOpenDocument(LPCTSTR lpszPathName)
{
	...
	*/
	...
	if(FAILED(hr))
		return FALSE;
	COleClientItemHook* pItem=(COleClientItemHook*)new COleClientItem(this);
...
BOOL CFormScript::OnSaveDocument(LPCTSTR lpszPathName)
{
	...
	//return m_pForm->SaveFile(lpszPathName);
	...
	if(pPersist!=NULL)
	{
		pPersist->Save(pStorage,FALSE);
		pPersist->SaveCompleted(NULL);
		/*
